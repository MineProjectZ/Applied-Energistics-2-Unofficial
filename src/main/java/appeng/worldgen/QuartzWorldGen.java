...
public final class QuartzWorldGen implements IWorldGenerator {
    ...
    public QuartzWorldGen() {
        ...
        this.oreNormal = new WorldGenMinable(
            ore, 0, AEConfig.instance.quartzOresPerCluster, Blocks.stone
        );
        this.oreCharged = new WorldGenMinable(
            charged, 0, AEConfig.instance.quartzOresPerCluster, Blocks.stone
        );
    }
    ...
    public void generate(
        ...
        final double oreDepthMultiplier
            = AEConfig.instance.quartzOresClusterAmount * seaLevel / 64;
        final int scale = (int) Math.round(
            r.nextGaussian() * Math.sqrt(oreDepthMultiplier) + oreDepthMultiplier
        );

        for (int x = 0; x < (r.nextBoolean() ? scale * 2 : scale) / 2; ++x) {
            final boolean isCharged
                = r.nextFloat() > AEConfig.instance.spawnChargedChance;
            final WorldGenMinable whichOre = isCharged ? this.oreCharged : this.oreNormal;

            if (WorldGenRegistry.INSTANCE.isWorldGenEnabled(
                    isCharged ? WorldGenType.ChargedCertusQuartz
                              : WorldGenType.CertusQuartz,
                    w
                )) {
                final int cx = chunkX * 16 + r.nextInt(22);
                final int cy = r.nextInt(40 * seaLevel / 64)
                    + r.nextInt(22 * seaLevel / 64) + 12 * seaLevel / 64;
