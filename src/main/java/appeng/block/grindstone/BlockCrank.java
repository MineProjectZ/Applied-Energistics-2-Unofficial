...
public class BlockCrank extends AEBaseTileBlock {
    ...
    public boolean isValidOrientation(
        ...
        return !(te instanceof TileCrank)
            || this.isCrankable(world, x, y, z, up.getOpposite());
    }

    private ForgeDirection
    findCrankable(final World world, final int x, final int y, final int z) {
    ...
    private boolean isCrankable(
        final World world,
        final int x,
        final int y,
        final int z,
        final ForgeDirection offset
    ) {
        final TileEntity te = world.getTileEntity(
            x + offset.offsetX, y + offset.offsetY, z + offset.offsetZ
        );

        return te instanceof ICrankable
            && ((ICrankable) te).canCrankAttach(offset.getOpposite());
    }
