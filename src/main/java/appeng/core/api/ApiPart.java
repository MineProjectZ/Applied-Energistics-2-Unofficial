...
public class ApiPart implements IPartHelper {
    ...
    private final Map<Class<?>, String> interfaces2Layer
    ...
    public Class getCombinedInstance(final String base) {
        ...
        final String description
            = base + ':' + Joiner.on(";").skipNulls().join(this.desc.iterator());
        ...
        for (final String name : this.desc) {
            try {
                ...
                myCLass = this.getClassByDesc(
                    Addendum, newPath, f, this.interfaces2Layer.get(Class.forName(name))
                );
                path = newPath;
    ...
    private Class getClassByDesc(
        final String addendum, final String fullPath, final String root, final String next
    ) {
        ...
        final DefaultPackageClassNameRemapper remapper
            = new DefaultPackageClassNameRemapper();
        ...
        try {
            ...
            if (!hasError) {
                AELog.info(
                    "Layer: " + n.name + " loaded successfully - " + size + " bytes"
                );
            }
        } catch (final Throwable t) {
    ...
    private Class loadClass(final String name, byte[] b) {
        ...
        try {
            ...
            final Method defineClassMethod = root.getDeclaredMethod(
                "defineClass", String.class, byte[].class, int.class, int.class
            );
            final Method runTransformersMethod = cls.getDeclaredMethod(
    ...
    public boolean registerNewLayer(final String layer, final String layerInterface) {
        try {
            ...
            if (this.interfaces2Layer.get(layerInterfaceClass) == null) {
                ...
                AELog.info(
                    "Layer " + layer + " not registered, " + layerInterface
                    + " already has a layer."
                );
            }
    ...
    public boolean placeBus(
        ...
        return PartPlacement.place(
            is, x, y, z, side, player, w, PartPlacement.PlaceType.PLACE_ITEM, 0
        );
    }
