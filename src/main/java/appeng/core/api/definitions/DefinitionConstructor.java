...
public class DefinitionConstructor {
    ...
    public DefinitionConstructor(
        final FeatureRegistry features, final FeatureHandlerRegistry handlers
    ) {
    ...
    final AEColoredItemDefinition
    constructColoredDefinition(final IItemDefinition target, final int offset) {
        ...
        for (final Item targetItem : target.maybeItem().asSet()) {
            for (final AEColor color : AEColor.VALID_COLORS) {
                ...
                definition.add(
                    color, new ItemStackSrc(targetItem, offset + color.ordinal(), state)
                );
            }
        }
    ...
    final AEColoredItemDefinition
    constructColoredDefinition(final ItemMultiPart target, final PartType type) {
        final ColoredItemDefinition definition = new ColoredItemDefinition();
