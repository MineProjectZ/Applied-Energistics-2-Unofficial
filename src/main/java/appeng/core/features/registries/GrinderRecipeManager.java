...
public final class GrinderRecipeManager implements IGrinderRegistry, IOreListener {
    ...
    public void addRecipe(final ItemStack in, final ItemStack out, final int cost) {
        ...
        this.log(
            "Allow Grinding of " + Platform.getItemDisplayName(in) + " to "
            + Platform.getItemDisplayName(out) + " for " + cost
        );
    ...
    public void addRecipe(
        ...
        this.log(
            "Allow Grinding of " + Platform.getItemDisplayName(in) + " to "
            + Platform.getItemDisplayName(out) + " with optional "
            + Platform.getItemDisplayName(optional) + " for " + cost
        );
        this.injectRecipe(new AppEngGrinderRecipe(
            this.copy(in), this.copy(out), this.copy(optional), chance, cost
        ));
    }
    ...
    private void injectRecipe(final AppEngGrinderRecipe appEngGrinderRecipe) {
        for (final IGrinderEntry gr : this.recipes) {
            if (Platform.isSameItemPrecise(
                    gr.getInput(), appEngGrinderRecipe.getInput()
                )) {
                return;
            }
        }
        ...
        if (input != null) {
            for (final IGrinderEntry r : this.recipes) {
                if (Platform.isSameItem(input, r.getInput())) {
                    this.log(
                        "Recipe for " + input.getUnlocalizedName() + " found "
    ...
    private void addDust(final String name, final ItemStack item) {
        ...
        if (this.dusts.containsKey(name)) {
            this.log(
                "Rejecting Dust - " + name + " : " + Platform.getItemDisplayName(item)
            );
            return;
        }
    ...
    public void oreRegistered(final String name, final ItemStack item) {
        if (name.startsWith("ore") || name.startsWith("crystal") || name.startsWith("gem")
            || name.startsWith("ingot") || name.startsWith("dust")) {
