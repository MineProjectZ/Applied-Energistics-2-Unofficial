...
public enum GuiBridge implements IGuiHandler {
    ...
    GUI_PORTABLE_CELL(
        ContainerMEPortableCell.class, IPortableCell.class, GuiHostType.ITEM, null
    ),

    GUI_WIRELESS_TERM(
        ContainerWirelessTerm.class,
        WirelessTerminalGuiObject.class,
        GuiHostType.ITEM,
        null
    ),
    ...
    GUI_CRAFTING_CPU(
        ContainerCraftingCPU.class,
        TileCraftingTile.class,
        GuiHostType.WORLD,
        SecurityPermissions.CRAFT
    ),

    GUI_NETWORK_TOOL(
        ContainerNetworkTool.class, INetworkTool.class, GuiHostType.ITEM, null
    ),

    GUI_QUARTZ_KNIFE(
        ContainerQuartzKnife.class, QuartzKnifeObj.class, GuiHostType.ITEM, null
    ),
    ...
    GUI_VIBRATION_CHAMBER(
        ContainerVibrationChamber.class,
    ...
    GUI_INTERFACE(
        ...
        IInterfaceHost.class,
        GuiHostType.WORLD,
        SecurityPermissions.BUILD
    ),

    GUI_BUS(
        ContainerUpgradeable.class,
        IUpgradeableHost.class,
        GuiHostType.WORLD,
        SecurityPermissions.BUILD
    ),

    GUI_IOPORT(
        ContainerIOPort.class,
        TileIOPort.class,
        GuiHostType.WORLD,
        SecurityPermissions.BUILD
    ),
    ...
    GUI_CRAFTING_TERMINAL(
        ContainerCraftingTerm.class,
        ICraftingTerminal.class,
        GuiHostType.WORLD,
        SecurityPermissions.CRAFT
    ),

    GUI_PATTERN_TERMINAL(
        ContainerPatternTerm.class,
    ...
    GUI_SPATIAL_IO_PORT(
        ContainerSpatialIOPort.class,
        TileSpatialIOPort.class,
        GuiHostType.WORLD,
        SecurityPermissions.BUILD
    ),
    ...
    GUI_CELL_WORKBENCH(
        ContainerCellWorkbench.class, TileCellWorkbench.class, GuiHostType.WORLD, null
    ),

    GUI_MAC(ContainerMAC.class, TileMolecularAssembler.class, GuiHostType.WORLD, null),

    GUI_CRAFTING_AMOUNT(
        ContainerCraftAmount.class,
        ITerminalHost.class,
    ...
    /**
     * I honestly wish I could just use the GuiClass Names myself, but I can't access them
     * without MC's Server Exploding.
     */
    private void getGui() {
        if (Platform.isClient()) {
            ...
            this.guiClass
                = ReflectionHelper.getClass(this.getClass().getClassLoader(), guiClass);
        ...
        if (ID.type.isTile()) {
            ...
            if (TE instanceof IPartHost) {
                ...
                if (ID.CorrectTileOrPart(part)) {
                    return this.updateGui(
                        ID.ConstructContainer(player.inventory, side, part),
                        w,
                        x,
                        y,
                        z,
                ...
                if (ID.CorrectTileOrPart(TE)) {
                    return this.updateGui(
                        ID.ConstructContainer(player.inventory, side, TE),
                        w,
                        x,
                        y,
                        z,
    ...
    public Object ConstructContainer(
        final InventoryPlayer inventory, final ForgeDirection side, final Object tE
    ) {
        try {
            ...
            if (target == null) {
                throw new IllegalStateException(
                    "Cannot find " + this.containerClass.getName() + "( "
    ...
    private Constructor findConstructor(
        final Constructor[] c, final InventoryPlayer inventory, final Object tE
    ) {
        for (final Constructor con : c) {
            final Class[] types = con.getParameterTypes();
            if (types.length == 2) {
                if (types[0].isAssignableFrom(inventory.getClass())
                    && types[1].isAssignableFrom(tE.getClass())) {
                    return con;
                }
            }
        }
    ...
    private void addPressAchievementToPlayer(
        ...
        final IMaterials possibleMaterials,
        final EntityPlayer player
    ) {
        ...
        final List<IComparableDefinition> presses
            = Lists.newArrayList(logic, eng, calc, silicon);
        ...
        if (Platform.hasPermissions(
            ...
            if (this.type.isItem()) {
                ...
                if (it != null && it.getItem() instanceof IGuiItem) {
                    final Object myItem
                        = ((IGuiItem) it.getItem()).getGuiObject(it, w, x, y, z);
