...
public class PlayerStatsRegistration {
    /**
     * {@link cpw.mods.fml.common.eventhandler.EventBus} to which the handlers might get
     * posted to depending if the feature is enabled
     */
    private final EventBus bus;

    /**
     * is true if the {@link appeng.core.features.AEFeature#Achievements} is enabled in
     * the
     */
    private final boolean isAchievementFeatureEnabled;

    /**
     * Constructs this with an {@link cpw.mods.fml.common.eventhandler.EventBus} and
    ...
    public PlayerStatsRegistration(final EventBus bus, final AEConfig config) {
        this.bus = bus;
        this.isAchievementFeatureEnabled
            = config.isFeatureEnabled(AEFeature.Achievements);
    }

    /**
     * Registers the {@link appeng.core.stats.AchievementCraftingHandler} and
     * {@link appeng.core.stats.AchievementPickupHandler} to the {@link #bus} if {@link
     * #isAchievementFeatureEnabled} is true.
     */
    public void registerAchievementHandlers() {
        if (this.isAchievementFeatureEnabled) {
            ...
            final AchievementCraftingHandler craftingHandler
                = new AchievementCraftingHandler(differentiator);
            final AchievementPickupHandler pickupHandler
                = new AchievementPickupHandler(differentiator);
    ...
    /**
     * Registers the {@link appeng.core.stats.AchievementHierarchy} and adds all {@link
     * appeng.core.stats.Achievements} to a new {@link
    ...
    public void registerAchievements() {
        if (this.isAchievementFeatureEnabled) {
            ...
            final AchievementPage ae2AchievementPage = new AchievementPage(
                "Applied Energistics 2", list.toArray(new Achievement[list.size()])
            );
