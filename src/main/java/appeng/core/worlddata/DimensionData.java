...
final class DimensionData
    implements IWorldDimensionData, IOnWorldStartable, IOnWorldStoppable {
    ...
    public void onWorldStart() {
        for (final Integer storageCellDimID : this.storageCellDimensionIDs) {
            DimensionManager.registerDimension(
                storageCellDimID, AEConfig.instance.storageProviderID
            );
        }
    ...
    public void addStorageCell(final int newStorageCellID) {
        ...
        DimensionManager.registerDimension(
            newStorageCellID, AEConfig.instance.storageProviderID
        );
    ...
    @Override
    public void setStoredSize(
        final int dim, final int targetX, final int targetY, final int targetZ
    ) {
        final String category = STORAGE_CELL_CATEGORY + dim;
    ...
    public void sendToPlayer(@Nullable final NetworkManager manager) {
        if (manager != null) {
            for (final int newDim :
                 this.config
                     .get(
                         PACKAGE_DEST_CATEGORY, PACKAGE_KEY_CATEGORY, PACKAGE_DEF_CATEGORY
                     )
                     .getIntList()) {
                manager.scheduleOutboundPacket(
                    (new PacketNewStorageDimension(newDim)).getProxy()
                );
            }
        } else {
