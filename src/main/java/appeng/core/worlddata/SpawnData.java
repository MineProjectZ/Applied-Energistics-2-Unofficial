...
final class SpawnData implements IWorldSpawnData {
    ...
    @Override
    public Collection<NBTTagCompound>
    getNearByMeteorites(final int dim, final int chunkX, final int chunkZ) {
    ...
    loadSpawnData(final int dim, final int chunkX, final int chunkZ) {
    ...
    private void writeSpawnData(
        final int dim, final int chunkX, final int chunkZ, final NBTTagCompound data
    ) {
        if (!Thread.holdsLock(SpawnData.class)) {
            throw new IllegalStateException("Invalid Request");
        }
