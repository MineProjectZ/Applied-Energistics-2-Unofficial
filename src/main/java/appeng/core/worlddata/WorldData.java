 ...
 * <p>
 * Data will change depending which world is loaded. Will probably not affect SMP at all
 * since only one world is loaded, but SSP more, cause they play on different worlds.
 *
 * @author thatsIch
...
public final class WorldData implements IWorldData {
    ...
    private WorldData(@Nonnull final File worldDirectory) {
        ...
        final CompassService compassService
            = new CompassService(this.compassDirectory, compassThreadFactory);
        final CompassData compassData
            = new CompassData(this.compassDirectory, compassService);
        ...
        this.startables = Lists.<IOnWorldStartable>newArrayList(
            playerData, dimensionData, storageData
        );
        this.stoppables
            = Lists.newArrayList(playerData, dimensionData, storageData, compassData);
    }
    ...
    private void onServerStarting() {
        ...
        if (!this.ae2directory.isDirectory() && !this.ae2directory.mkdir()) {
            throw new IllegalStateException(
                "Failed to create " + this.ae2directory.getAbsolutePath()
            );
        }
        ...
        if (!this.compassDirectory.isDirectory() && !this.compassDirectory.mkdir()) {
            throw new IllegalStateException(
                "Failed to create " + this.compassDirectory.getAbsolutePath()
            );
        }
        ...
        if (!this.spawnDirectory.isDirectory() && !this.spawnDirectory.mkdir()) {
            throw new IllegalStateException(
                "Failed to create " + this.spawnDirectory.getAbsolutePath()
            );
        }
