...
public final class Registration {
    ...
    void preInitialize(final FMLPreInitializationEvent event) {
        ...
        RecipeSorter.register(
            "AE2-Shapeless", ShapelessRecipe.class, Category.SHAPELESS, ""
        );
        ...
        for (final IAEFeature feature :
             definitions.getFeatureRegistry().getRegisteredFeatures()) {
            feature.postInit();
        }
    }
    ...
    private void registerSpatial(final boolean force) {
        ...
        if (this.storageBiome == null) {
            if (force && config.storageBiomeID == -1) {
                config.storageBiomeID
                    = Platform.findEmpty(BiomeGenBase.getBiomeGenArray());
    ...
    private void assignMaterials(final Materials target, final IMaterials source) {
        ...
        target.materialCell128SpatialPart
            = this.converter.of(source.cell128SpatialPart());

        target.materialSilicon = this.converter.of(source.silicon());
        target.materialSkyDust = this.converter.of(source.skyDust());

        target.materialCalcProcessorPress
            = this.converter.of(source.calcProcessorPress());
        target.materialEngProcessorPress = this.converter.of(source.engProcessorPress());
        target.materialLogicProcessorPress
            = this.converter.of(source.logicProcessorPress());

        target.materialCalcProcessorPrint
            = this.converter.of(source.calcProcessorPrint());
        target.materialEngProcessorPrint = this.converter.of(source.engProcessorPrint());
        target.materialLogicProcessorPrint
            = this.converter.of(source.logicProcessorPrint());
        ...
        target.materialPurifiedCertusQuartzCrystal
            = this.converter.of(source.purifiedCertusQuartzCrystal());
        target.materialPurifiedNetherQuartzCrystal
            = this.converter.of(source.purifiedNetherQuartzCrystal());
        target.materialPurifiedFluixCrystal
            = this.converter.of(source.purifiedFluixCrystal());
        ...
        target.materialCertusQuartzCrystal
            = this.converter.of(source.certusQuartzCrystal());
        target.materialCertusQuartzCrystalCharged
            = this.converter.of(source.certusQuartzCrystalCharged());
    ...
    private void assignBlocks(final Blocks target, final IBlocks source) {
        ...
        target.blockQuartzGrowthAccelerator
            = this.converter.of(source.quartzGrowthAccelerator());
    ...
    void initialize(
        ...
        final Runnable recipeLoader
            = new RecipeLoader(recipeDirectory, customRecipeConfig, this.recipeHandler);
        recipeLoader.run();
        ...
        partHelper.registerNewLayer(
            "appeng.parts.layers.LayerIFluidHandler",
        ...
        final PlayerStatsRegistration registration = new PlayerStatsRegistration(
            FMLCommonHandler.instance().bus(), AEConfig.instance
        );
    ...
    void postInit(final FMLPostInitializationEvent event) {
        ...
        for (final Item wirelessTerminalItem :
            ...
            registries.wireless().registerWirelessHandler((IWirelessTermHandler
            ) wirelessTerminalItem);
        }

        if (AEConfig.instance.isFeatureEnabled(AEFeature.ChestLoot)) {
            final ChestGenHooks d
        ...
        for (final WorldGenType type : WorldGenType.values()) {
            registries.worldgen()
                .disableWorldGenForProviderID(type, StorageWorldProvider.class);
        ...
        for (final int dimension : AEConfig.instance.meteoriteDimensionWhitelist) {
            registries.worldgen().enableWorldGenForDimension(
                WorldGenType.Meteorites, dimension
            );
        }
