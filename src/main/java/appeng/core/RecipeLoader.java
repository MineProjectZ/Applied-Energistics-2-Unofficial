...
/**
 * handles the decision if recipes should be loaded from jar, loaded from file system or
 * force copied from jar
 *
 * @author thatsIch
...
public class RecipeLoader implements Runnable {
     ...
     */
    private static final String ASSETS_RECIPE_PATH
    ...
    public final void run() {
        if (this.config.isEnabled()) {
            // setup copying
            final RecipeResourceCopier copier
                = new RecipeResourceCopier("assets/appliedenergistics2/recipes/");
            ...
            try {
                ...
                // parse recipes prioritising the user scripts by using the generated as
                // template
                this.handler.parseRecipes(
                    new ConfigLoader(generatedRecipesDir, userRecipesDir), "index.recipe"
                );
            }
            // on failure use jar parsing
                ...
                AELog.debug(e);
                this.handler.parseRecipes(
                    new JarLoader(ASSETS_RECIPE_PATH), "index.recipe"
                );
