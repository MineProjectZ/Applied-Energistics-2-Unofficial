...
public class MetaRotation implements IOrientable {
    ...
    public ForgeDirection getUp() {
        return ForgeDirection.getOrientation(
            this.w.getBlockMetadata(this.x, this.y, this.z)
        );
    }
    ...
    public void setOrientation(final ForgeDirection forward, final ForgeDirection up) {
        if (this.w instanceof World) {
            ((World) this.w)
                .setBlockMetadataWithNotify(this.x, this.y, this.z, up.ordinal(), 1 + 2);
        } else {
            throw new IllegalStateException(
                this.w.getClass().getName() + " received, expected World"
            );
        }
    }
}
