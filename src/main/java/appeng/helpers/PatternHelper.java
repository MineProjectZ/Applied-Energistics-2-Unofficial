...
public class PatternHelper implements ICraftingPatternDetails, Comparable<PatternHelper> {
    ...
    public PatternHelper(final ItemStack is, final World w) {
        ...
        for (int x = 0; x < inTag.tagCount(); x++) {
            final ItemStack gs
                = ItemStack.loadItemStackFromNBT(inTag.getCompoundTagAt(x));
        ...
        if (this.isCrafting) {
            ...
            for (int x = 0; x < outTag.tagCount(); x++) {
                final ItemStack gs
                    = ItemStack.loadItemStackFromNBT(outTag.getCompoundTagAt(x));
        ...
        final Map<IAEItemStack, IAEItemStack> tmpOutputs
            = new HashMap<IAEItemStack, IAEItemStack>();
        ...
        final Map<IAEItemStack, IAEItemStack> tmpInputs
            = new HashMap<IAEItemStack, IAEItemStack>();
    ...
    private void markItemAs(final int slotIndex, final ItemStack i, final TestStatus b) {
        ...
        (b == TestStatus.ACCEPT ? this.passCache : this.failCache)
            .add(new TestLookup(slotIndex, i));
    }
    ...
    isValidItemForSlot(final int slotIndex, final ItemStack i, final World w) {
        if (!this.isCrafting) {
        ...
        if (this.standardRecipe.matches(this.testFrame, w)) {
            final ItemStack testOutput
                = this.standardRecipe.getCraftingResult(this.testFrame);

            if (Platform.isSameItemPrecise(this.correctOutput, testOutput)) {
                this.testFrame.setInventorySlotContents(
                    slotIndex, this.crafting.getStackInSlot(slotIndex)
                );
            ...
            final ItemStack testOutput
                = CraftingManager.getInstance().findMatchingRecipe(this.testFrame, w);
    ...
    private enum TestStatus { ACCEPT, DECLINE, TEST }
