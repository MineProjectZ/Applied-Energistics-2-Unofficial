...
public class CraftingWatcher implements ICraftingWatcher {
    ...
    public CraftingWatcher(
        final CraftingGridCache cache, final ICraftingWatcherHost host
    ) {
        this.gsc = cache;
        this.host = host;
    }
    ...
    public boolean add(final IAEStack e) {
        ...
        return this.myInterests.add(e.copy())
            && this.gsc.getInterestManager().put(e, this);
    }

    @Override
    public boolean remove(final Object o) {
        return this.myInterests.remove(o)
            && this.gsc.getInterestManager().remove((IAEStack) o, this);
    }
    ...
    private class ItemWatcherIterator implements Iterator<IAEStack> {
        ...
        public ItemWatcherIterator(
            final CraftingWatcher parent, final Iterator<IAEStack> i
        ) {
            this.watcher = parent;
        ...
        @Override
        public void remove() {
            CraftingWatcher.this.gsc.getInterestManager().remove(
                this.myLast, this.watcher
            );
