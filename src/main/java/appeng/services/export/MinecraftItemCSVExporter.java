...
final class MinecraftItemCSVExporter implements Exporter {
    ...
    private static final String VERBOSE_HEADER
        = MINIMAL_HEADER + ", Unlocalized Name, Is Air?, Class Name";
    private static final String EXPORT_SUCCESSFUL_MESSAGE
        = "Exported successfully %d items into %s";
    private static final String EXPORT_UNSUCCESSFUL_MESSAGE
        = "Exporting was unsuccessful.";
     ...
     * @param itemRegistry    the registry with minecraft items. Needs to be populated at
     *     that time, thus the exporting can
     *                        only happen in init (pre-init is the
     *                        phase when all items are determined)
     * @param mode            mode in which the export should be operated. Resulting CSV
     *     will change depending on this.
     */
    ...
    public void export() {
        ...
        final List<String> lines = Lists.transform(
            itemList, new ItemRowExtractFunction(this.itemRegistry, this.mode)
        );
        ...
        try {
            ...
            final String header
                = this.mode == ExportMode.MINIMAL ? MINIMAL_HEADER : VERBOSE_HEADER;
            writer.write(header);
            writer.write("\n");
            writer.write(joined);
            writer.flush();
    ...
    private static final class TypeExtractFunction
        ...
        private TypeExtractFunction(
            @Nonnull final String itemName, @Nonnull final ExportMode mode
        ) {
        ...
        public String apply(@Nullable final ItemStack input) {
            if (input == null) {
                ...
                AELog.debug(
                    EXTRACTING_ITEM_MESSAGE, input.getDisplayName(), input.getItemDamage()
                );
            }
    ...
    private static final class ItemRowExtractFunction implements Function<Item, String> {
        ...
        private static final String EXPORTING_SUBTYPES_MESSAGE
            = "Exporting input %s with subtypes: %b";
        private static final String EXPORTING_SUBTYPES_FAILED_MESSAGE
            = "Could not export subtypes of: %s";
        ...
        public String apply(@Nullable final Item input) {
            ...
            if (hasSubtypes) {
                ...
                try {
                    ...
                    AELog.warn(
                        EXPORTING_SUBTYPES_FAILED_MESSAGE, input.getUnlocalizedName()
                    );
                    AELog.debug(ignored);
                ...
                final List<String> transformedTypes = Lists.transform(
                    stacks, new TypeExtractFunction(itemName, this.mode)
                );
            ...
            final List<String> joinedBlockAttributes = Lists.newArrayListWithCapacity(5);
            final String unlocalizedItem = input.getUnlocalizedName();
            final String localization = StatCollector.translateToLocal(
