...
public final class VersionChecker implements Runnable {
     ...
     * @param lastAfterInterval last version check including the interval defined in the
     *     config
     */
    private void processInterval(final long nowInMs, final long lastAfterInterval) {
        if (nowInMs > lastAfterInterval) {
            ...
            final VersionFetcher modFetcher
                = new ModVersionFetcher(rawModVersion, parser);
    ...
    private void processVersions(
        @Nonnull final Version modVersion, @Nonnull final FormattedRelease githubRelease
    ) {
        ...
        if (githubVersion.isNewerAs(modVersion)) {
            ...
            if (this.config.shouldNotifyPlayer()) {
                AELog.info(
                    "Newer version is available: " + ghFormatted + " (found) > "
                    + modFormatted + " (current)"
                );
            ...
            AELog.info(
                "No newer version is available: " + ghFormatted + "(found) < "
                + modFormatted + " (current)"
            );
        }
    }
    ...
    /**
     * Checks if the version checker mod is installed and handles it depending on that
     * information
     *
    ...
    private void interactWithVersionCheckerMod(
        @Nonnull final String modFormatted,
        @Nonnull final String ghFormatted,
        ...
        if (Loader.isModLoaded("VersionChecker")) {
            ...
            versionInf.setString(
                "newFileName", "appliedenergistics2-" + ghFormatted + ".jar"
            );
            FMLInterModComms.sendRuntimeMessage(
                AppEng.instance(), "VersionChecker", "addUpdate", versionInf
            );
