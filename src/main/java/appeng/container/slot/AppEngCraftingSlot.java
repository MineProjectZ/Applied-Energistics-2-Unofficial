...
public class AppEngCraftingSlot extends AppEngSlot {
    ...
    /**
     * the itemStack passed in is the output - ie, iron ingots, and pickaxes, not ore and
     * wood. Typically increases an internal count then calls onCrafting(item).
     */
    @Override
    ...
    protected void onCrafting(final ItemStack par1ItemStack) {
        par1ItemStack.onCrafting(
            this.thePlayer.worldObj, this.thePlayer, this.amountCrafted
        );
        ...
        if (par1ItemStack.getItem() instanceof ItemPickaxe
            && ((ItemTool) par1ItemStack.getItem()).func_150913_i()
    ...
    onPickupFromSlot(final EntityPlayer par1EntityPlayer, final ItemStack par2ItemStack) {
        FMLCommonHandler.instance().firePlayerCraftingEvent(
            par1EntityPlayer, par2ItemStack, this.craftMatrix
        );
            ...
            if (itemstack1 != null) {
                ...
                if (itemstack1.getItem().hasContainerItem(itemstack1)) {
                    final ItemStack itemstack2
                        = itemstack1.getItem().getContainerItem(itemstack1);

                    if (itemstack2 != null && itemstack2.isItemStackDamageable()
                        && itemstack2.getItemDamage() > itemstack2.getMaxDamage()) {
                        MinecraftForge.EVENT_BUS.post(
                            new PlayerDestroyItemEvent(this.thePlayer, itemstack2)
                        );
                        continue;
                    }

                    if (!itemstack1.getItem().doesContainerItemLeaveCraftingGrid(
                            itemstack1
                        )
