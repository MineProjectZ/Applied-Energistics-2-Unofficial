...
public class ContainerUpgradeable extends AEBaseContainer implements IOptionalSlotHost {
    ...
    protected void setupConfig() {
        ...
        if (this.supportCapacity()) {
            ...
            this.addSlotToContainer(
                new OptionalSlotFakeTypeOnly(inv, this, 8, x, y, 1, 1, 2)
            );
        }
    }
    ...
    private void checkToolbox() {
        if (this.hasToolbox()) {
            ...
            if (currentItem != this.tbInventory.getItemStack()) {
                if (currentItem != null) {
                    if (Platform.isSameItem(
                            this.tbInventory.getItemStack(), currentItem
                        )) {
                        this.getPlayerInv().setInventorySlotContents(
                            this.tbSlot, this.tbInventory.getItemStack()
                        );
                    } else {
