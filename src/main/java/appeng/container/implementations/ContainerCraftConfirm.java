    ...
    public void detectAndSendChanges() {
        ...
        if (this.getJob() != null && this.getJob().isDone()) {
            try {
                ...
                try {
                    final PacketMEInventoryUpdate a
                        = new PacketMEInventoryUpdate((byte) 0);
                    final PacketMEInventoryUpdate b
                        = new PacketMEInventoryUpdate((byte) 1);
                    final PacketMEInventoryUpdate c = this.result.isSimulation()
                    ...
                    for (final IAEItemStack out : plan) {
                        ...
                        if (c != null && this.result.isSimulation()) {
                            m = o.copy();
                            o = items.extractItems(
                                o, Actionable.SIMULATE, this.getActionSource()
                            );
                ...
                this.getPlayerInv().player.addChatMessage(
                    new ChatComponentText("Error: " + e.toString())
                );
                AELog.debug(e);
        ...
        if (this.result != null && !this.isSimulation()) {
            ...
            if (g != null && originalGui != null && this.getOpenContext() != null) {
                NetworkHandler.instance.sendTo(
                    new PacketSwitchGuis(originalGui),
