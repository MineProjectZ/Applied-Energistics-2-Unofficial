...
public final class ContainerInterfaceTerminal extends AEBaseContainer {
    ...
    private final Map<IInterfaceHost, InvTracker> diList
        = new HashMap<IInterfaceHost, InvTracker>();
    private final Map<Long, InvTracker> byId = new HashMap<Long, InvTracker>();
    private IGrid grid;
    private NBTTagCompound data = new NBTTagCompound();

    public ContainerInterfaceTerminal(
        final InventoryPlayer ip, final PartInterfaceTerminal anchor
    ) {
        super(ip, anchor);
            ...
            if (agn != null && agn.isActive()) {
                for (final IGridNode gn : this.grid.getMachines(TileInterface.class)) {
                    if (gn.isActive()) {
                        ...
                        if (ih.getInterfaceDuality().getConfigManager().getSetting(
        ...
        if (total != this.diList.size() || missing) {
            ...
            for (final Entry<IInterfaceHost, InvTracker> en : this.diList.entrySet()) {
                ...
                for (int x = 0; x < inv.server.getSizeInventory(); x++) {
                    if (this.isDifferent(
                            inv.server.getStackInSlot(x), inv.client.getStackInSlot(x)
                        )) {
        ...
        if (!this.data.hasNoTags()) {
            try {
                NetworkHandler.instance.sendTo(
                    new PacketCompressedNBT(this.data),
        ...
        if (inv != null) {
            ...
            switch (action) {
                case PICKUP_OR_SET_DOWN:
                    ...
                    if (hasItemInHand) {
                        ...
                        if (inSlot == null) {
                            ...
                            player.inventory.setItemStack(
                                interfaceSlot.addItems(inHand.copy())
                            );
                        ...
                        mySlot.setInventorySlotContents(
                            0, playerHand.addItems(mySlot.getStackInSlot(0))
                        );
                    }
                ...
                case SHIFT_CLICK:
                    ...
                    mySlot.setInventorySlotContents(
                        0, playerInv.addItems(mySlot.getStackInSlot(0))
                    );
                ...
                case MOVE_REGION:
                    ...
                    for (int x = 0; x < inv.server.getSizeInventory(); x++) {
                        inv.server.setInventorySlotContents(
                            x, playerInvAd.addItems(inv.server.getStackInSlot(x))
                        );
                    }
            ...
            if (agn != null && agn.isActive()) {
                for (final IGridNode gn : this.grid.getMachines(TileInterface.class)) {
                    ...
                    if (gn.isActive()
                        ...
                        this.diList.put(
                            ih,
                            new InvTracker(dual, dual.getPatterns(), dual.getTermName())
                        );
                    }
                }
    ...
    private void addItems(
        ...
        for (int x = 0; x < length; x++) {
            ...
            // "update" client side.
            inv.client.setInventorySlotContents(
                x + offset, is == null ? null : is.copy()
            );
    ...
    private static class InvTracker {
        ...
        public InvTracker(
            ...
            this.client
                = new AppEngInternalInventory(null, this.server.getSizeInventory());
