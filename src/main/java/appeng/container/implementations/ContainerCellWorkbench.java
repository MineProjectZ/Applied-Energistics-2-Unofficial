...
public class ContainerCellWorkbench extends ContainerUpgradeable {
    ...
    public void setFuzzy(final FuzzyMode valueOf) {
        ...
        if (cwi != null) {
            cwi.setFuzzyMode(
                this.workBench.getInventoryByName("cell").getStackInSlot(0), valueOf
            );
        }
    }

    public void nextWorkBenchCopyMode() {
        this.workBench.getConfigManager().putSetting(
            Settings.COPY_MODE, Platform.nextEnum(this.getWorkBenchCopyMode())
        );
    }
    ...
    protected void setupConfig() {
        ...
        for (int w = 0; w < 7; w++) {
            for (int z = 0; z < 9; z++) {
                this.addSlotToContainer(
                    new SlotFakeTypeOnly(inv, offset, x + z * 18, y + w * 18)
                );
                offset++;
            }
        }
         ...
         * z++) addSlotToContainer( new OptionalSlotFakeTypeOnly( inv, this, offset++, x,
         * y, z, w, 1 ) ); for (int w = 0; w < 2; w++) for (int z = 0; z < 9; z++)
         * addSlotToContainer( new OptionalSlotFakeTypeOnly( inv, this, offset++, x, y, z,
         * w + 2, 2 ) ); for (int w = 0; w < 2; w++) for (int z = 0; z < 9; z++)
         * addSlotToContainer( new OptionalSlotFakeTypeOnly( inv, this, offset++, x, y, z,
         * w + 4, 3 ) ); }
         */
    }
    ...
    public void detectAndSendChanges() {
        ...
        if (Platform.isServer()) {
            if (this.workBench.getWorldObj().getTileEntity(
                    this.workBench.xCoord, this.workBench.yCoord, this.workBench.zCoord
                )
                != this.workBench) {
                ...
                if (this.prevStack != is) {
                    ...
                    for (final Object s : this.inventorySlots) {
                        if (s instanceof OptionalSlotRestrictedInput) {
                            final OptionalSlotRestrictedInput sri
                                = (OptionalSlotRestrictedInput) s;
                            icrafting.sendSlotContents(
                                this, sri.slotNumber, sri.getStack()
                            );
                        }
                    }
    ...
    onUpdate(final String field, final Object oldValue, final Object newValue) {
        if (field.equals("copyMode")) {
            this.workBench.getConfigManager().putSetting(
                Settings.COPY_MODE, this.getCopyMode()
            );
        }
    ...
    private FuzzyMode getWorkBenchFuzzyMode() {
        ...
        if (cwi != null) {
            return cwi.getFuzzyMode(
                this.workBench.getInventoryByName("cell").getStackInSlot(0)
            );
        }
    ...
    private class Upgrades implements IInventory {
        ...
        public boolean isItemValidForSlot(final int i, final ItemStack itemstack) {
            return ContainerCellWorkbench.this.getCellUpgradeInventory()
