...
public class ContainerInscriber
    extends ContainerUpgradeable implements IProgressProvider {
        ...
        if (s == this.middle) {
            ...
            for (final IInscriberRecipe recipe :
                ...
                final boolean matchA
                    = (top == null && !recipe.getTopOptional().isPresent())
                    || (Platform.isSameItemPrecise(top, recipe.getTopOptional().orNull()))
                        && // and...
                ...
                final boolean matchB
                    = (bot == null && !recipe.getTopOptional().isPresent())
                    || (Platform.isSameItemPrecise(bot, recipe.getTopOptional().orNull()))
                        && // and...
        ...
        if ((s == this.top && bot != null) || (s == this.bottom && top != null)) {
            ...
            for (final IInscriberRecipe recipe :
                ...
                if (Platform.isSameItemPrecise(
                        recipe.getTopOptional().orNull(), otherSlot
                    )) {
                    isValid = Platform.isSameItemPrecise(
                        is, recipe.getBottomOptional().orNull()
                    );
                } else if (Platform.isSameItemPrecise(
                               recipe.getBottomOptional().orNull(), otherSlot
                           )) {
                    isValid = Platform.isSameItemPrecise(
