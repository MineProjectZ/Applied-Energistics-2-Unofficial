...
public abstract class AEBaseContainer extends Container {
    ...
    private final List<PacketPartialItem> dataChunks
        = new LinkedList<PacketPartialItem>();
    ...
    public AEBaseContainer(
        final InventoryPlayer ip, final TileEntity myTile, final IPart myPart
    ) {
        ...
        for (final Field f : this.getClass().getFields()) {
            if (f.isAnnotationPresent(GuiSync.class)) {
                ...
                if (this.syncData.containsKey(annotation.value())) {
                    AELog.warn(
                        "Channel already in use: " + annotation.value() + " for "
                        + f.getName()
                    );
                } else {
                    this.syncData.put(
                        annotation.value(), new SyncData(this, f, annotation)
                    );
                }
            }
        }
    ...
    public AEBaseContainer(final InventoryPlayer ip, final Object anchor) {
        ...
        if (this.tileEntity == null && this.part == null && this.obj == null) {
            throw new IllegalArgumentException(
                "Must have a valid anchor, instead " + anchor + " in " + ip
            );
        }
    ...
    private void parsePartials() {
        ...
        try {
            final NBTTagCompound data
                = CompressedStreamTools.readCompressed(new ByteArrayInputStream(buffer));
            if (data != null) {
                this.setTargetStack(AEApi.instance().storage().createItemStack(
    ...
    public void setTargetStack(final IAEItemStack stack) {
        ...
        if (Platform.isClient()) {
            ...
            final ItemStack b = this.clientRequestedTargetItem == null
                ? null
            ...
            try {
                ...
                final ByteArrayInputStream bis
                    = new ByteArrayInputStream(data, 0, stream.size());
                while (bis.available() > 0) {
                    final int nextBLock
                        = bis.available() > maxChunkSize ? maxChunkSize : bis.available();
                ...
                for (final byte[] packet : miniPackets) {
                    final PacketPartialItem ppi
                        = new PacketPartialItem(page, miniPackets.size(), packet);
                    page++;
    ...
    public void
    verifyPermissions(final SecurityPermissions security, final boolean requirePower) {
        if (Platform.isClient()) {
            return;
        }
        ...
        this.setValidContainer(
            this.isValidContainer() && this.hasAccess(security, requirePower)
        );
    }

    protected boolean
    hasAccess(final SecurityPermissions perm, final boolean requirePower) {
    ...
    protected void bindPlayerInventory(
        final InventoryPlayer inventoryPlayer, final int offsetX, final int offsetY
    ) {
        // bind player inventory
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
                if (this.locked.contains(j + i * 9 + 9)) {
                    this.addSlotToContainer(new SlotDisabled(
                        inventoryPlayer,
                        j + i * 9 + 9,
                        8 + j * 18 + offsetX,
                        offsetY + i * 18
        ...
        for (int i = 0; i < 9; i++) {
            if (this.locked.contains(i)) {
                this.addSlotToContainer(new SlotDisabled(
                    inventoryPlayer, i, 8 + i * 18 + offsetX, 58 + offsetY
                ));
            } else {
                this.addSlotToContainer(new SlotPlayerHotBar(
                    inventoryPlayer, i, 8 + i * 18 + offsetX, 58 + offsetY
                ));
            }
        }
    ...
    public ItemStack transferStackInSlot(final EntityPlayer p, final int idx) {
        ...
        final AppEngSlot clickSlot
            = (AppEngSlot) this.inventorySlots.get(idx); // require AE SLots!
            ...
            if (clickSlot.isPlayerSide()) {
                ...
                for (final Object inventorySlot : this.inventorySlots) {
                    ...
                    if (!(cs.isPlayerSide()) && !(cs instanceof SlotFake)
                        && !(cs instanceof SlotCraftingMatrix)) {
                    ...
                    case SPLIT_OR_PLACE_SINGLE:
                        ...
                        if (is != null) {
                            if (hand == null) {
                                ...
                                is.stackSize
                                    = Math.min(is.getMaxStackSize(), is.stackSize + 1);
                            } else {
                                is = hand.copy();
                                is.stackSize = 1;
                            }
        ...
        switch (action) {
                ...
                if (isg != null && releaseQty > 0) {
                    ...
                    if (ais == null) {
                        ...
                        final ItemStack fail
                            = ia.removeItems(1, extracted.getItemStack(), null);
                        if (fail == null) {
                            this.getCellInventory().extractItems(
                                extracted, Actionable.MODULATE, this.getActionSource()
                            );
                        }
                        ...
                        if (ais != null) {
                            ...
                            if (fail != null) {
                                this.getCellInventory().injectItems(
                                    ais, Actionable.MODULATE, this.getActionSource()
                                );
                            }
            ...
            case PICKUP_OR_SET_DOWN:
                ...
                if (player.inventory.getItemStack() == null) {
                    ...
                    IAEItemStack ais = AEApi.instance().storage().createItemStack(
            ...
            case SPLIT_OR_PLACE_SINGLE:
                ...
                if (player.inventory.getItemStack() == null) {
                    if (slotItem != null) {
                        ...
                        ais = this.getCellInventory().extractItems(
                            ais, Actionable.SIMULATE, this.getActionSource()
                        );
