...
public final class ASMTweaker implements IClassTransformer {
    ...
    private final Multimap<String, PublicLine> privateToPublicMethods
        = HashMultimap.create();
    ...
    public ASMTweaker() {
        ...
        this.privateToPublicMethods.put(
            "appeng.tile.AEBaseTile", new PublicLine("b", "(Ldh;)V")
        );
        ...
        this.privateToPublicMethods.put(
            "appeng.tile.AEBaseTile", new PublicLine("a", "(Ldh;)V")
        );
    }
    ...
    public byte[] transform(
        ...
        try {
            if (transformedName != null
                && this.privateToPublicMethods.containsKey(transformedName)) {
                final ClassNode classNode = new ClassNode();
                final ClassReader classReader = new ClassReader(basicClass);
                classReader.accept(classNode, 0);

                for (final PublicLine set :
                     this.privateToPublicMethods.get(transformedName)) {
                ...
                if (transformedName.equals(
                    ...
                    for (final MethodNode mn : classNode.methods) {
                        if (mn.name.equals("func_146977_a")
                            || (mn.name.equals("a") && mn.desc.equals("(Lzk;)V"))) {
                            ...
                            newNode.instructions.add(new MethodInsnNode(
                    ...
                    for (final MethodNode mn : classNode.methods) {
                        if (mn.name.equals("func_73863_a") || mn.name.equals("drawScreen")
                            ...
                            final Iterator<AbstractInsnNode> i
        ...
        for (final MethodNode mn : classNode.methods) {
            if (mn.name.equals(set.name) && mn.desc.equals(set.desc)) {
                mn.access
                    = (mn.access
                       & (~(
                           Opcodes.ACC_FINAL | Opcodes.ACC_PRIVATE | Opcodes.ACC_PROTECTED
                       )))
                    | Opcodes.ACC_PUBLIC;
