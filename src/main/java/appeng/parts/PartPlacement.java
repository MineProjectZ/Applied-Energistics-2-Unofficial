...
public class PartPlacement {
    ...
    public static boolean place(
        ...
        if (held != null && Platform.isWrench(player, held, x, y, z)
            && player.isSneaking()) {
            ...
            if (host != null) {
                if (!world.isRemote) {
                    final LookDirection dir
                        = Platform.getPlayerRay(player, getEyeOffset(player));
                    final MovingObjectPosition mop
                        = block.collisionRayTrace(world, x, y, z, dir.getA(), dir.getB());
                    if (mop != null) {
                        ...
                        if (host.canAddPart(held, side)) {
                            if (host.getFacadeContainer().addFacade(fp)) {
                                ...
                                if (!player.capabilities.isCreativeMode) {
                                    ...
                                    if (held.stackSize == 0) {
                                        player.inventory
                                            .mainInventory[player.inventory.currentItem]
                                            = null;
                                        MinecraftForge.EVENT_BUS.post(
                                            new PlayerDestroyItemEvent(player, held)
                                        );
                                    }
                                }
                                return true;
        ...
        if (host == null && tile != null
            && IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.FMP)) {
            host = ((IFMP) IntegrationRegistry.INSTANCE.getInstance(IntegrationType.FMP))
                       .getOrCreateHost(tile);
        }
                            ...
                            if (world.isRemote) {
                                NetworkHandler.instance.sendToServer(
                                    new PacketPartPlacement(
                                        x, y, z, face, getEyeOffset(player)
                                    )
                                );
                            }
                            return true;
        ...
        if (host == null && pass == PlaceType.PLACE_ITEM) {
            ...
            if (host == null && tile != null
                && IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.FMP)) {
                host = ((IFMP
                        ) IntegrationRegistry.INSTANCE.getInstance(IntegrationType.FMP))
                           .getOrCreateHost(tile);
            }
            ...
            final Optional<ItemBlock> maybeMultiPartItemBlock
                = multiPart.maybeItemBlock();

            final boolean hostIsNotPresent = host == null;
            final boolean multiPartPresent = maybeMultiPartBlock.isPresent()
                && maybeMultiPartStack.isPresent() && maybeMultiPartItemBlock.isPresent();
            final boolean canMultiPartBePlaced
                = maybeMultiPartBlock.get().canPlaceBlockAt(world, te_x, te_y, te_z);
        ...
        if (!host.canAddPart(held, side)) {
            if (pass == PlaceType.INTERACT_FIRST_PASS || pass == PlaceType.PLACE_ITEM) {
                ...
                if ((blkID == null || blkID.isReplaceable(world, te_x, te_y, te_z)
                     || host != null)
    ...
    selectPart(final EntityPlayer player, final IPartHost host, final Vec3 pos) {
    ...
    public void playerInteract(final PlayerInteractEvent event) {
        if (event.action == Action.RIGHT_CLICK_AIR
            ...
            // are really stupid...
            final MovingObjectPosition mop
                = Platform.rayTrace(event.entityPlayer, true, false);
    ...
    public enum PlaceType { PLACE_ITEM, INTERACT_FIRST_PASS, INTERACT_SECOND_PASS }
}