...
/**
 * The most basic class for any part reporting information, like terminals or monitors.
 * This can also include basic panels which just provide light. <p> It deals with the most
 * basic functionalities like network data, grid registration or the rotation of the
 * actual part. <p> The direct abstract subclasses are usually a better entry point for
 * adding new concrete ones. But this might be an ideal starting point to completely new
 * type, which does not resemble any existing one.
 *
 * @author AlgorithmX2
 * @author yueh
 * @version rv3
...
public abstract class AbstractPartReporting
    extends AEBasePart implements IPartMonitor, IPowerChannelState {
    ...
    public void writeToStream(final ByteBuf data) throws IOException {
        ...
        try {
            if (this.getProxy().getEnergy().isNetworkPowered()) {
                this.clientFlags
                    = this.getClientFlags() | AbstractPartReporting.POWERED_FLAG;
            }

            if (this.getProxy().getPath().isNetworkBooting()) {
                this.clientFlags
                    = this.getClientFlags() | AbstractPartReporting.BOOTING_FLAG;
            }

            if (this.getProxy().getNode().meetsChannelRequirements()) {
                this.clientFlags
                    = this.getClientFlags() | AbstractPartReporting.CHANNEL_FLAG;
            }
    ...
    public boolean onPartActivate(final EntityPlayer player, final Vec3 pos) {
        ...
        if (!player.isSneaking()
            && Platform.isWrench(
                player, player.inventory.getCurrentItem(), te.xCoord, te.yCoord, te.zCoord
            )) {
    ...
    public final void onPlacement(
        ...
        final byte rotation = (byte
        ) (MathHelper.floor_double((player.rotationYaw * 4F) / 360F + 2.5D) & 3);
    ...
    public final boolean isPowered() {
        try {
            if (Platform.isServer()) {
                ...
                return (
                    (this.getClientFlags() & PartPanel.POWERED_FLAG)
    ...
    public final boolean isActive() {
        if (!this.isLightSource()) {
            return (
                (this.getClientFlags() & (PartPanel.CHANNEL_FLAG | PartPanel.POWERED_FLAG)
                )
