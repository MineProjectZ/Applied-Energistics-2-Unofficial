...
public abstract class AbstractPartMonitor
    extends AbstractPartDisplay implements IPartStorageMonitor, IStackWatcherHost {
    private static final IWideReadableNumberConverter NUMBER_CONVERTER
    ...
    public boolean onPartActivate(final EntityPlayer player, final Vec3 pos) {
        ...
        if (Platform.isWrench(player, eq, te.xCoord, te.yCoord, te.zCoord)) {
            ...
            player.addChatMessage((this.isLocked ? PlayerMessages.isNowLocked
    ...
    private void updateReportingValue(final IMEMonitor<IAEItemStack> itemInventory) {
        if (this.configuredItem != null) {
            final IAEItemStack result
                = itemInventory.getStorageList().findPrecise(this.configuredItem);
            if (result == null) {
    ...
    public void onStackChange(
        ...
        if (this.configuredItem != null) {
            ...
            final String humanReadableText
                = NUMBER_CONVERTER.toWideReadableForm(stackSize);
