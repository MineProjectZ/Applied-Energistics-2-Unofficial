 ...
 * <p>
 * Note this applies only to terminals like the ME Terminal. It does not apply for more
 * specialized terminals like the Interface Terminal.
 *
 * @author AlgorithmX2
 * @author yueh
 * @version rv3
    ...
    public boolean onPartActivate(final EntityPlayer player, final Vec3 pos) {
        if (!super.onPartActivate(player, pos)) {
            if (!player.isSneaking()) {
                ...
                Platform.openGUI(
                    player, this.getHost().getTile(), this.getSide(), this.getGui(player)
                );
