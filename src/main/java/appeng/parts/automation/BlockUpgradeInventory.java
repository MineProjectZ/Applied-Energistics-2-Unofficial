...
public class BlockUpgradeInventory extends UpgradeInventory {
    ...
    public BlockUpgradeInventory(
        final Block block, final IAEAppEngInventory parent, final int s
    ) {
        super(parent, s);
        this.block = block;
    }
    ...
    public int getMaxInstalled(final Upgrades upgrades) {
        ...
        for (final ItemStack is : upgrades.getSupported().keySet()) {
            ...
            if (encodedItem instanceof ItemBlock
                && Block.getBlockFromItem(encodedItem) == this.block) {
