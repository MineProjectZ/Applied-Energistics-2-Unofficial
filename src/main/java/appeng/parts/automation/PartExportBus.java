...
public class PartExportBus extends PartSharedItemBus implements ICraftingRequester {
    private final MultiCraftingTracker craftingTracker
        = new MultiCraftingTracker(this, 9);
    ...
    protected TickRateModulation doBusWork() {
        ...
        try {
            ...
            final SchedulingMode schedulingMode
                = (SchedulingMode) this.getConfigManager().getSetting(
            ...
            if (destination != null) {
                ...
                for (x = 0; x < this.availableSlots() && this.itemToSend > 0; x++) {
                    ...
                    final IAEItemStack ais
                        = this.getConfig().getAEStackInSlot(slotToExport);
                    ...
                    if (this.getInstalledUpgrades(Upgrades.FUZZY) > 0) {
                        for (final IAEItemStack o : ImmutableList.copyOf(
                                 inv.getStorageList().findFuzzy(ais, fzMode)
                             )) {
    ...
    public TickingRequest getTickingRequest(final IGridNode node) {
        return new TickingRequest(
            TickRates.ExportBus.getMin(),
            TickRates.ExportBus.getMax(),
            this.isSleeping(),
            false
        );
    }
    ...
    public IAEItemStack injectCraftedItems(
        ...
        try {
            if (d != null && this.getProxy().isActive()) {
                ...
                if (energy.extractAEPower(power, mode, PowerMultiplier.CONFIG)
                    > power - 0.01) {
    ...
    private void pushItemIntoTarget(
        ...
        if (canFit > 0) {
            ...
            final IAEItemStack itemsToAdd
                = Platform.poweredExtraction(energy, inv, ais, this.mySrc);
