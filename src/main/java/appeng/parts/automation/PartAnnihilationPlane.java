...
public class PartAnnihilationPlane
    extends PartBasicState implements IGridTickable, IWorldCallable<TickRateModulation> {
    private static final IIcon SIDE_ICON = CableBusTextures.PartPlaneSides.getIcon();
    private static final IIcon BACK_ICON
        = CableBusTextures.PartTransitionPlaneBack.getIcon();
    private static final IIcon STATUS_ICON
        = CableBusTextures.PartMonitorSidesStatus.getIcon();
    private static final IIcon ACTIVE_ICON
        = CableBusTextures.BlockAnnihilationPlaneOn.getIcon();
    ...
    public void getBoxes(final IPartCollisionHelper bch) {
        ...
        if (host != null) {
            ...
            if (this.isAnnihilationPlane(
                    te.getWorldObj().getTileEntity(
                        x - e.offsetX, y - e.offsetY, z - e.offsetZ
                    ),
                    this.getSide()
                )) {
                minX = 0;
            ...
            if (this.isAnnihilationPlane(
                    te.getWorldObj().getTileEntity(
                        x - u.offsetX, y - u.offsetY, z - u.offsetZ
                    ),
                    this.getSide()
                )) {
                minY = 0;
    ...
    public void onEntityCollision(final Entity entity) {
        if (this.isAccepting && entity instanceof EntityItem && !entity.isDead
            && Platform.isServer() && this.getProxy().isActive()) {
            boolean capture = false;

            // This is the middle point of the entities BB, which is better suited for
            // comparisons that don't rely on it "touching" the plane
            double posYMiddle
            ...
            switch (this.getSide()) {
                case DOWN:
                case UP:
                    if (entity.posX > this.getTile().xCoord
                        && entity.posX < this.getTile().xCoord + 1) {
                        if (entity.posZ > this.getTile().zCoord
                            && entity.posZ < this.getTile().zCoord + 1) {
                            if ((entity.posY > this.getTile().yCoord + 0.9
                                 && this.getSide() == ForgeDirection.UP)
                ...
                case NORTH:
                    if (entity.posX > this.getTile().xCoord
                        && entity.posX < this.getTile().xCoord + 1) {
                        if (posYMiddle > this.getTile().yCoord
                            && posYMiddle < this.getTile().yCoord + 1) {
                            if ((entity.posZ > this.getTile().zCoord + 0.9
                                 && this.getSide() == ForgeDirection.SOUTH)
                ...
                case WEST:
                    if (entity.posZ > this.getTile().zCoord
                        ...
                        if (posYMiddle > this.getTile().yCoord
                            && posYMiddle < this.getTile().yCoord + 1) {
                            if ((entity.posX > this.getTile().xCoord + 0.9
                                 && this.getSide() == ForgeDirection.EAST)
                ...
                if (changed) {
                    ServerHelper.proxy.sendToAllNearExcept(
                        ...
                        new PacketTransitionEffect(
                            entity.posX, entity.posY, entity.posZ, this.getSide(), false
                        )
                    );
                }
            }
    ...
    /**
     * Stores an {@link EntityItem} inside the network and either marks it as dead or sets
     * it to the leftover stackSize.
     *
     ...
     * It will update the entity to match the leftover stack size as well as mark it as
     * dead without any leftover amount.
     *
     ...
     */
    private boolean
    handleOverflow(final EntityItem entityItem, final IAEItemStack overflow) {
    ...
    private void spawnOverflow(final IAEItemStack overflow) {
        ...
        final EntityItem overflowEntity
            = new EntityItem(w, x, y, z, overflow.getItemStack());
        ...
        if (this.isAccepting && this.getProxy().isActive()) {
            try {
                ...
                if (this.canHandleBlock(w, x, y, z)) {
                    ...
                    final float requiredPower
                        = this.calculateEnergyUsage(w, x, y, z, items);
     ...
     */
    private boolean
    canHandleBlock(final WorldServer w, final int x, final int y, final int z) {
        ...
        if (permissionCache == YesNo.UNDECIDED) {
            BlockEvent.BreakEvent event = new BlockEvent.BreakEvent(
                x, y, z, w, block, w.getBlockMetadata(x, y, z), player
            );
            MinecraftForge.EVENT_BUS.post(event);
            permissionCache = (event.isCanceled()) ? YesNo.NO : YesNo.YES;
        }
        return permissionCache == YesNo.YES && !ignoreMaterials && !ignoreBlocks
            && !w.isAirBlock(x, y, z) && w.blockExists(x, y, z)
            && w.canMineBlock(player, x, y, z) && hardness >= 0f;
    }
     ...
     * @return true, if the network can store at least a single item of all drops or no
     *     drops are reported
     */
    private boolean canStoreItemStacks(final List<ItemStack> itemStacks) {
        ...
        try {
            ...
            for (final ItemStack itemStack : itemStacks) {
                ...
                if (overflow == null
                    || itemToTest.getStackSize() > overflow.getStackSize()) {
                    canStore = true;
                }
            }
        } catch (final GridAccessException e) {
    ...
    private void breakBlockAndStoreItems(
        ...
        final AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
            x - 0.2, y - 0.2, z - 0.2, x + 1.2, y + 1.2, z + 1.2
        );
