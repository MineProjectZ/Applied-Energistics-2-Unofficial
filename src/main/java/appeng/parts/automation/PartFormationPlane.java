...
public class PartFormationPlane extends PartUpgradeable
    ...
    public void renderStatic(
        ...
        rh.setTexture(
            ...
            CableBusTextures.PartPlaneSides.getIcon(),
            CableBusTextures.PartTransitionPlaneBack.getIcon(),
            isActive ? CableBusTextures.BlockFormPlaneOn.getIcon()
                     : this.getItemStack().getIconIndex(),
            CableBusTextures.PartPlaneSides.getIcon(),
            CableBusTextures.PartPlaneSides.getIcon()
        );
    ...
    private boolean
    isTransitionPlane(final TileEntity blockTileEntity, final ForgeDirection side) {
    ...
    public IAEItemStack injectItems(
        ...
        final YesNo placeBlock
            = (YesNo) this.getConfigManager().getSetting(Settings.PLACE_BLOCK);
        ...
        if (w.getBlock(x, y, z).isReplaceable(w, x, y, z)) {
            if (placeBlock == YesNo.YES
                && (i instanceof ItemBlock || i instanceof IPlantable
                    || i instanceof ItemSkull || i instanceof ItemFirework
                    || i instanceof IPartItem || i instanceof ItemReed)) {
                ...
                if (sum < AEConfig.instance.formationPlaneEntityLimit) {
                    if (type == Actionable.MODULATE) {
                        ...
                        final EntityItem ei = new EntityItem(
                            w,
                            ((side.offsetX != 0 ? 0.0 : 0.7)
                             * (Platform.getRandomFloat() - 0.5f))
                                + 0.5 + side.offsetX * -0.3 + x,
                            ((side.offsetY != 0 ? 0.0 : 0.7)
                             * (Platform.getRandomFloat() - 0.5f))
                                + 0.5 + side.offsetY * -0.3 + y,
                            ((side.offsetZ != 0 ? 0.0 : 0.7)
                             * (Platform.getRandomFloat() - 0.5f))
                                + 0.5 + side.offsetZ * -0.3 + z,
                            is.copy()
                        );
