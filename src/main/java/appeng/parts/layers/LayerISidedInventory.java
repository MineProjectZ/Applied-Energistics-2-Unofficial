 ...
 * <p>
 * this is considerably more complicated then the other wrappers as it requires creating a
 * "unified inventory". <p> You must use {@link ISidedInventory} instead of {@link
 * IInventory}. <p> If your inventory changes in between placement and removal, you must
 * trigger a PartChange on the {@link IPartHost} so it can recalculate the inventory
 * wrapper.
 */
public class LayerISidedInventory extends LayerBase implements ISidedInventory {
    ...
    public void notifyNeighbors() {
        ...
        if (inventories.isEmpty() || slotCount == 0) {
            ...
            sideData = new int[][] { NULL_SIDES, NULL_SIDES, NULL_SIDES,
                                     NULL_SIDES, NULL_SIDES, NULL_SIDES };
            ...
            for (final ISidedInventory sides : inventories) {
                ...
                final int[] cSidesList = sideData[currentSide.ordinal()]
                    = new int[slotCount];
    ...
    @Override
    public boolean
    canInsertItem(final int slot, final ItemStack itemstack, final int side) {
        if (this.invLayer == null) {
            return false;
        }
    ...
    @Override
    public boolean
    canExtractItem(final int slot, final ItemStack itemstack, final int side) {
        if (this.invLayer == null) {
            return false;
        }
