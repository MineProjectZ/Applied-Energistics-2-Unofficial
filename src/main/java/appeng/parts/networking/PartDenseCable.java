...
public class PartDenseCable extends PartCable {
    ...
    public void renderInventory(final IPartRenderHelper rh, final RenderBlocks renderer) {
        ...
        OffsetIcon main
            = new OffsetIcon(this.getTexture(this.getCableColor()), offU, offV);
        OffsetIcon ch1
            = new OffsetIcon(this.getChannelTex(4, false).getIcon(), offU, offV);
        OffsetIcon ch2
            = new OffsetIcon(this.getChannelTex(4, true).getIcon(), offU, offV);

        for (final ForgeDirection side :
             EnumSet.of(ForgeDirection.UP, ForgeDirection.DOWN)) {
        ...
        for (final ForgeDirection side :
             EnumSet.of(ForgeDirection.EAST, ForgeDirection.WEST)) {
            rh.renderInventoryFace(main, side, renderer);
            rh.renderInventoryFace(ch1, side, renderer);
            rh.renderInventoryFace(ch2, side, renderer);
        }
        ...
        for (final ForgeDirection side :
             EnumSet.of(ForgeDirection.SOUTH, ForgeDirection.NORTH)) {
            rh.renderInventoryFace(main, side, renderer);
            rh.renderInventoryFace(ch1, side, renderer);
            rh.renderInventoryFace(ch2, side, renderer);
        }
    ...
    public void renderStatic(
        ...
        this.setRenderCache(
            rh.useSimplifiedRendering(x, y, z, this, this.getRenderCache())
        );
        ...
        if (sides.size() != 2 || !this.nonLinear(sides) || hasBuses) {
            for (final ForgeDirection of : this.getConnections()) {
                if (this.isDense(of)) {
                    this.renderDenseConnection(
                        x, y, z, rh, renderer, this.getChannelsOnSide()[of.ordinal()], of
                    );
                } else if (this.isSmart(of)) {
                    this.renderSmartConnection(
                        x, y, z, rh, renderer, this.getChannelsOnSide()[of.ordinal()], of
                    );
                } else {
                    this.renderCoveredConnection(
                        x, y, z, rh, renderer, this.getChannelsOnSide()[of.ordinal()], of
                    );
                }
            }
            ...
            switch (selectedSide) {
                ...
                case UP:
                    renderer.setRenderBounds(
                        3 / 16.0, 0, 3 / 16.0, 13 / 16.0, 16 / 16.0, 13 / 16.0
                    );
                    ...
                    Tessellator.instance.setColorOpaque_I(
                        this.getCableColor().blackVariant
                    );
                    rh.setTexture(
                        firstIcon,
                        firstIcon,
                    ...
                    this.renderAllFaces(
                        (AEBaseBlock) rh.getBlock(), x, y, z, rh, renderer
                    );

                    Tessellator.instance.setColorOpaque_I(
                        this.getCableColor().whiteVariant
                    );
                    rh.setTexture(
                        secondIcon,
                        secondIcon,
                ...
                case WEST:
                    ...
                    final FlippableIcon ico
                        = blk.getRendererInstance().getTexture(ForgeDirection.EAST);
                    ico.setFlip(false, true);

                    renderer.setRenderBounds(
                        0, 3 / 16.0, 3 / 16.0, 16 / 16.0, 13 / 16.0, 13 / 16.0
                    );
                    rh.renderBlockCurrentBounds(x, y, z, renderer);
                    ...
                    rh.setTexture(
                        firstOffset, firstOffset, firstOffset, firstOffset, firstIcon, fpA
                    );
                    this.renderAllFaces(
                        (AEBaseBlock) rh.getBlock(), x, y, z, rh, renderer
                    );
                ...
                case SOUTH:
                    ...
                    renderer.uvRotateWest = 1;
                    renderer.setRenderBounds(
                        3 / 16.0, 3 / 16.0, 0, 13 / 16.0, 13 / 16.0, 16 / 16.0
                    );
                    rh.renderBlockCurrentBounds(x, y, z, renderer);
    ...
    private void renderDenseConnection(
        ...
        final TileEntity te = this.getTile().getWorldObj().getTileEntity(
            x + of.offsetX, y + of.offsetY, z + of.offsetZ
        );
        ...
        /*
         * ( ghh != null && partHost != null && ghh.getCableConnectionType( of ) ==
         * AECableType.GLASS && partHost.getPart( of.getOpposite() ) == null ) { isGlass =
         * true; rh.setTexture( getGlassTexture( myColor = partHost.getColor() ) ); } else
         * if ( partHost == null && ghh != null && ghh.getCableConnectionType( of ) !=
         * AECableType.GLASS ) { rh.setTexture( getSmartTexture( myColor ) ); switch (of)
         * { case DOWN: rh.setBounds( 3, 0, 3, 13, 4, 13 ); break; case EAST:
         * rh.setBounds( 12, 3, 3, 16, 13, 13 ); break; case NORTH: rh.setBounds( 3, 3, 0,
         * 13, 13, 4
         * ); break; case SOUTH: rh.setBounds( 3, 3, 12, 13, 13, 16 ); break; case UP:
         * rh.setBounds( 3, 12, 3, 13, 16, 13 ); break; case WEST: rh.setBounds( 0, 3, 3,
         * 4, 13, 13 ); break; default: return; } rh.renderBlock( x, y, z, renderer ); if
         * ( true ) { setSmartConnectionRotations( of, renderer ); IIcon firstIcon = new
         * TaughtIcon( getChannelTex( channels, false ).getIcon(), -0.2f ); IIcon
         * secondIcon = new TaughtIcon( getChannelTex( channels, true
         * ).getIcon(),
         * -0.2f );
         * if ( of == ForgeDirection.EAST || of == ForgeDirection.WEST ) { AEBaseBlock blk
         * = (AEBaseBlock) rh.getBlock(); FlippableIcon ico =
         * blk.getRendererInstance().getTexture( ForgeDirection.EAST ); ico.setFlip(
         * false, true ); }
         * Tessellator.INSTANCE.setBrightness( 15 << 20 | 15 << 5 );
         * Tessellator.INSTANCE.setColorOpaque_I( myColor.mediumVariant ); rh.setTexture(
         * firstIcon, firstIcon, firstIcon, firstIcon, firstIcon, firstIcon );
         * renderAllFaces( (AEBaseBlock)
         * rh.getBlock(), x, y, z, renderer );
         * Tessellator.INSTANCE.setColorOpaque_I( myColor.whiteVariant ); rh.setTexture(
         * secondIcon, secondIcon, secondIcon, secondIcon, secondIcon, secondIcon );
    ...
    private boolean isSmart(final ForgeDirection of) {
        ...
        if (te instanceof IGridHost) {
            final AECableType t
                = ((IGridHost) te).getCableConnectionType(of.getOpposite());
