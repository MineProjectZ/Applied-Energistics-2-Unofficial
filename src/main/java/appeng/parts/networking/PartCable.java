...
public class PartCable extends AEBasePart implements IPartCable {
    ...
    public PartCable(final ItemStack is) {
        ...
        this.getProxy().setColor(AEColor.values(
        )[((ItemMultiPart) is.getItem()).variantOf(is.getItemDamage())]);
    }
    ...
    public boolean changeColor(final AEColor newColor, final EntityPlayer who) {
        if (this.getCableColor() != newColor) {
            ...
            try {
                hasPermission = this.getProxy().getSecurity().hasPermission(
                    who, SecurityPermissions.BUILD
                );
    ...
    public void renderStatic(
        ...
        for (final ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (p instanceof IGridHost) {
                ...
                final IPartHost partHost
                    = te instanceof IPartHost ? (IPartHost) te : null;
                final IGridHost gh = te instanceof IGridHost ? (IGridHost) te : null;
                if (partHost == null && gh != null
                    && gh.getCableConnectionType(dir) != AECableType.GLASS) {
                ...
                switch (of) {
                    ...
                    case UP:
                        renderer.setRenderBounds(
                            6 / 16.0, 0, 6 / 16.0, 10 / 16.0, 16 / 16.0, 10 / 16.0
                        );
                        break;
                    case EAST:
                    case WEST:
                        ...
                        renderer.setRenderBounds(
                            0, 6 / 16.0, 6 / 16.0, 16 / 16.0, 10 / 16.0, 10 / 16.0
                        );
                        break;
                    case NORTH:
                    case SOUTH:
                        ...
                        renderer.setRenderBounds(
                            6 / 16.0, 6 / 16.0, 0, 10 / 16.0, 10 / 16.0, 16 / 16.0
                        );
                        break;
                    default:
                }
    ...
    public boolean readFromStream(final ByteBuf data) throws IOException {
        ...
        for (final ForgeDirection d : ForgeDirection.values()) {
            if (d == ForgeDirection.UNKNOWN) {
                ...
                // This works only because cables are always read *last* from the packet
                // update for a cable bus
        ...
        return !myC.equals(this.getConnections()) || wasPowered != this.powered
            || channelsChanged;
    }
    ...
    protected boolean nonLinear(final EnumSet<ForgeDirection> sides) {
        return (sides.contains(ForgeDirection.EAST) && sides.contains(ForgeDirection.WEST)
            ...
            || (sides.contains(ForgeDirection.NORTH)
                && sides.contains(ForgeDirection.SOUTH))
    ...
    IIcon getSmartTexture(final AEColor c) {
        ...
        final IParts parts = AEApi.instance().definitions().parts();
        final ItemStack smartCableStack
            = parts.cableSmart().stack(AEColor.Transparent, 1);

        return parts.cableCovered()
            .item(AEColor.Transparent)
            .getIconIndex(smartCableStack);
    }

    @SideOnly(Side.CLIENT)
    protected void
    setSmartConnectionRotations(final ForgeDirection of, final RenderBlocks renderer) {
        switch (of) {
            case UP:
            case DOWN:
    ...
    protected void renderAllFaces(
        ...
        rh.renderFace(
            ...
            y,
            z,
            blk.getRendererInstance().getTexture(ForgeDirection.UP),
            ForgeDirection.UP,
            renderer
        );
    }
