...
public class PartP2PTunnelME
    extends PartP2PTunnel<PartP2PTunnelME> implements IGridTickable {
    private final Connections connection = new Connections(this);
    private final AENetworkProxy outerProxy
        = new AENetworkProxy(this, "outer", null, true);
    ...
    public PartP2PTunnelME(final ItemStack is) {
        ...
        this.outerProxy.setFlags(
            GridFlags.DENSE_CAPACITY, GridFlags.CANNOT_CARRY_COMPRESSED
        );
    }
    ...
    public TickingRequest getTickingRequest(final IGridNode node) {
        return new TickingRequest(
            TickRates.METunnel.getMin(), TickRates.METunnel.getMax(), true, false
        );
    }
            ...
            if (!this.getProxy().getPath().isNetworkBooting()) {
                if (!this.getProxy().getEnergy().isNetworkPowered()) {
                    ...
                    TickHandler.INSTANCE.addCallable(
                        this.getTile().getWorldObj(), this.connection
                    );
                } else {
    ...
    public void updateConnections(final Connections connections) {
        if (connections.isDestroy()) {
            ...
            while (i.hasNext()) {
                ...
                try {
                    if (cw.getTunnel().getProxy().getGrid()
            ...
            final LinkedList<PartP2PTunnelME> newSides
                = new LinkedList<PartP2PTunnelME>();
            try {
                for (final PartP2PTunnelME me : this.getOutputs()) {
                    if (me.getProxy().isActive()
                        && connections.getConnections().get(me.getGridNode()) == null) {
                        newSides.add(me);
                    }
                }
