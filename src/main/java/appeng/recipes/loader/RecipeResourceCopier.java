...
/**
 * copies recipes in jars onto file system includes the readme, needs to be modified if
 * other files needs to be handled
 *
 * @author thatsIch
...
public class RecipeResourceCopier {
     ...
     * @throws IOException              {@see #getResourceListing} and if copying the
     *     detected resource to file is not possible
     ...
     * @throws URISyntaxException {@see #getResourceListing}
     * @throws IOException        {@see #getResourceListing} and if copying the detected
     *     resource to file is not possible
    ...
    private void copyTo(
        ...
        for (final String list : listing) {
            if (list.endsWith(identifier)) {
                // generate folder before the file is copied so no empty folders will be
                // generated
    ...
    private void copyFile(
        ...
        final InputStream inStream
            = copierClass.getResourceAsStream('/' + directory + fileName);
    ...
    /**
     * List directory contents for a resource folder. Not recursive. This is basically a
     * brute-force implementation. Works for regular files and also JARs.
     *
     * @param clazz Any java class that lives in the same place as the resources you want.
     * @param path  Should end with "/", but not start with one.
     * @return Just the name of each member item, not the full paths.
     * @throws URISyntaxException            if it is a file path and the URL can not be
     *     converted to URI
     ...
     */
    @Nonnull
    private String[] getResourceListing(
        @Nonnull final Class<?> clazz, @Nonnull final String path
    ) throws URISyntaxException, IOException {
        assert clazz != null;
        assert path != null;
        ...
        if (dirURL != null) {
            ...
            if (protocol.equals(JAR_PROTOCOL)) {
                ...
                final String jarPath
                    = dirPath.substring(5, dirPath.indexOf('!')); // strip out only
                // the JAR file
                final JarFile jar
                    = new JarFile(URLDecoder.decode(jarPath, UTF_8_ENCODING));
                try {
                    final Enumeration<JarEntry> entries
                        = jar.entries(); // gives ALL entries in jar
                    final Collection<String> result
                        = new HashSet<String>(INITIAL_RESOURCE_CAPACITY
                        ); // avoid duplicates
