...
public abstract class InscriberProcess implements ICraftHandler, IWebsiteSerializer {
        ...
        if (output.size() == 1 && output.get(0).size() == 1) {
            if (input.size() == 1 && input.get(0).size() > 1) {
                ...
                throw new RecipeError(
                    "Inscriber recipes cannot have rows, and must have more then one input."
                );
            }
        } else {
    ...
    @Override
    public boolean canCraft(final ItemStack reqOutput)
        throws RegistrationError, MissingIngredientError {
        return this.output != null
            && Platform.isSameItemPrecise(this.output.getItemStack(), reqOutput);
    }
