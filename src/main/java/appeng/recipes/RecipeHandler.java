...
public class RecipeHandler implements IRecipeHandler {
    ...
    private String getName(final ItemStack is) throws RecipeError {
        ...
        if (maybeCrystalSeedItem.isPresent()
            && is.getItem() == maybeCrystalSeedItem.get()) {
    ...
    public void injectRecipes() {
        if (cpw.mods.fml.common.Loader.instance().hasReachedState(
        ...
        for (final Entry<Class, Integer> e : processed.entrySet()) {
            AELog.info(
                "Recipes Loading: " + e.getKey().getSimpleName() + ": " + e.getValue()
                + " loaded."
            );
        }
        ...
        if (AEConfig.instance.isFeatureEnabled(AEFeature.WebsiteRecipes)) {
            try {
                final ZipOutputStream out
                    = new ZipOutputStream(new FileOutputStream("recipes.zip"));

                final HashMultimap<String, IWebsiteSerializer> combined
                    = HashMultimap.create();
                ...
                for (final String realName : combined.keySet()) {
                    ...
                    for (final IWebsiteSerializer ws : combined.get(realName)) {
                        ...
                        if (rew != null && rew.length() > 0) {
                            out.putNextEntry(
                                new ZipEntry(realName + '_' + offset + ".txt")
                            );
                            offset++;
    ...
    private List<IWebsiteSerializer> findRecipe(final ItemStack output) {
        ...
        for (final ICraftHandler ch : this.data.handlers) {
            try {
                if (ch instanceof IWebsiteSerializer
                    && ((IWebsiteSerializer) ch).canCraft(output)) {
    ...
    private void
    processTokens(final IRecipeLoader loader, final String file, final int line)
        throws RecipeError {
        try {
            ...
            if (split != -1) {
                final String operation
                    = this.tokens.remove(0).toLowerCase(Locale.ENGLISH);
                ...
                if (operation.equals("alias")) {
                    if (this.tokens.size() == 3 && this.tokens.indexOf("->") == 1) {
                        ...
                        throw new RecipeError(
                            "Alias must have exactly 1 input and 1 output."
                        );
                    }
                } else if (operation.equals("group")) {
                    final List<String> pre = this.tokens.subList(0, split - 1);
                    final List<String> post
                        = this.tokens.subList(split, this.tokens.size());

                    final List<List<IIngredient>> inputs = this.parseLines(pre);

                    if (inputs.size() == 1 && inputs.get(0).size() > 0
                        && post.size() == 1) {
                        this.data.groups.put(
                            post.get(0),
                            new GroupIngredient(post.get(0), inputs.get(0), 1)
                        );
                    } else {
                        throw new RecipeError(
                            "Group must have exactly 1 output, and 1 or more inputs."
                        );
                    }
                    ...
                    if (inputs.size() == 1 && inputs.get(0).size() > 0
                        && post.size() == 1) {
                        final ICraftHandler ch
                            = new OreRegistration(inputs.get(0), post.get(0));
                        this.addCrafting(ch);
                    } else {
                        throw new RecipeError(
                ...
                if (operation.equals("exceptions")
                    ...
                    if (this.tokens.size() == 1) {
                        ...
                        throw new RecipeError(
                            "exceptions must be true or false explicitly."
                        );
                    }
                }
				else if( operation.equals( "crash" ) && ( this.tokens.get( 0 ).equals( "true" ) || this.tokens.get( 0 ).equals( "false" ) ) )
				{
                    ...
                    if (this.tokens.size() == 1
                        ...
                        throw new RecipeError(
                            "erroronmissing must be true or false explicitly."
                        );
                    }
                } else if (operation.equals("import")) {
                    if (this.tokens.size() == 1) {
                        (new RecipeHandler(this))
                            .parseRecipes(loader, this.tokens.get(0));
                    } else {
                        throw new RecipeError("Import must have exactly 1 input.");
                    }
                } else {
                    throw new RecipeError(
                        operation + ": " + this.tokens.toString()
            ...
            AELog.warn(
                "Recipe Error '" + e.getMessage() + "' near line:" + line + " in " + file
                + " with: " + this.tokens.toString()
            );
            if (this.data.exceptions) {
                AELog.debug(e);
            }
    ...
    private List<List<IIngredient>> parseLines(final Iterable<String> subList)
        throws RecipeError {
