...
public class TickHandler {
    ...
    private final Queue<IWorldCallable<?>> serverQueue
        = new LinkedList<IWorldCallable<?>>();
    ...
    private final HashMap<Integer, PlayerColor> cliPlayerColors
        = new HashMap<Integer, PlayerColor>();
    private final HashMap<Integer, PlayerColor> srvPlayerColors
        = new HashMap<Integer, PlayerColor>();
    ...
    public void onTick(final TickEvent ev) {
        if (ev.type == Type.CLIENT && ev.phase == Phase.START) {
            ...
            final CableRenderMode currentMode
                = AEApi.instance().partHelper().getCableRenderMode();
        ...
        if (ev.type == Type.WORLD && ev.phase == Phase.END) {
            ...
            synchronized (this.craftingJobs) {
                ...
                if (!jobSet.isEmpty()) {
                    final int simTime = Math.max(
                        1,
                        AEConfig.instance.craftingCalculationTimePerTick / jobSet.size()
                    );
    ...
    public void
    registerCraftingSimulation(final World world, final CraftingJob craftingJob) {
        synchronized (this.craftingJobs) {
