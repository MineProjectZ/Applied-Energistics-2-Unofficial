...
public class FacadeContainer implements IFacadeContainer {
    ...
    public void rotateLeft() {
        ...
        newFacades[ForgeDirection.UP.ordinal()]
            = this.storage.getFacade(ForgeDirection.UP.ordinal());
        newFacades[ForgeDirection.DOWN.ordinal()]
            = this.storage.getFacade(ForgeDirection.DOWN.ordinal());

        newFacades[ForgeDirection.EAST.ordinal()]
            = this.storage.getFacade(ForgeDirection.NORTH.ordinal());
        newFacades[ForgeDirection.SOUTH.ordinal()]
            = this.storage.getFacade(ForgeDirection.EAST.ordinal());

        newFacades[ForgeDirection.WEST.ordinal()]
            = this.storage.getFacade(ForgeDirection.SOUTH.ordinal());
        newFacades[ForgeDirection.NORTH.ordinal()]
            = this.storage.getFacade(ForgeDirection.WEST.ordinal());
        ...
        for (int x = 0; x < this.facades; x++) {
            ...
            if ((facadeSides & ix) == ix) {
                ...
                if (isBC
                    ...
                    final IFacadePart created = bc.createFacadePart(
                        (Block) Block.blockRegistry.getObjectById(ids[0]), ids[1], side
                    );
                    ...
                    for (final Item facadeItem : AEApi.instance()
                        ...
                        if (facade != null) {
                            ...
                            this.storage.setFacade(
                                x, ifa.createPartFromItemStack(facade, side)
                            );
                        }
                    }
                }
                        ...
                        this.storage.setFacade(
                            ...
                            ((IFacadeItem) i)
                                .createPartFromItemStack(
                                    is, ForgeDirection.getOrientation(x)
                                )
                        );
                    } else {
                        if (IntegrationRegistry.INSTANCE.isEnabled(
                                IntegrationType.BuildCraftTransport
                            )) {
                            final IBuildCraftTransport bc
