...
public final class EntityTinyTNTPrimed
    ...
    public void onUpdate() {
        ...
        {
            for (final ItemStack tntStack :
                 AEApi.instance().definitions().blocks().tinyTNT().maybeStack(1).asSet(
                 )) {
                final EntityItem item = new EntityItem(
                    this.worldObj, this.posX, this.posY, this.posZ, tntStack
                );
        ...
        if (this.fuse <= 0) {
            ...
            this.worldObj.spawnParticle(
                "smoke", this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D
            );
        }
        this.fuse--;
    }
    ...
    void explode() {
        this.worldObj.playSoundEffect(
            ...
            4.0F,
            (1.0F
             + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.2F)
                * 32.9F
        );
        ...
        if (AEConfig.instance.isFeatureEnabled(AEFeature.TinyTNTBlockDamage)) {
            this.posY -= 0.25;
            final Explosion ex = new Explosion(
                this.worldObj, this, this.posX, this.posY, this.posZ, 0.2f
            );
