...
public class TileMolecularAssembler extends AENetworkInvTile
    implements IUpgradeableHost, IConfigManagerHost, IGridTickable, ICraftingMachine,
               IPowerChannelState {
    ...
    public TileMolecularAssembler() {
        ...
        this.upgrades
            = new DefinitionUpgradeInventory(assembler, this, this.getUpgradeSlots());
    ...
    public boolean pushPattern(
        final ICraftingPatternDetails patternDetails,
        final InventoryCrafting table,
    ...
    public void readFromNBT_TileMolecularAssembler(final NBTTagCompound data) {
        if (data.hasKey("myPlan")) {
            final ItemStack myPat
                = ItemStack.loadItemStackFromNBT(data.getCompoundTag("myPlan"));
            ...
            if (myPat != null && myPat.getItem() instanceof ItemEncodedPattern) {
                ...
                if (ph != null && ph.isCraftable()) {
                    ...
                    this.myPlan = ph;
                    this.pushDirection
                        = ForgeDirection.getOrientation(data.getInteger("pushDirection"));
                }
            }
        }
    ...
    tickingRequest(final IGridNode node, int ticksSinceLastCall) {
        ...
        if (this.progress >= 100) {
            ...
            final ItemStack output
                = this.myPlan.getOutput(this.craftingInv, this.getWorldObj());
            if (output != null) {
    ...
    private void ejectHeldItems() {
        if (this.inv.getStackInSlot(9) == null) {
            for (int x = 0; x < 9; x++) {
                ...
                if (is != null) {
                    if (this.myPlan == null
                        || !this.myPlan.isValidItemForSlot(x, is, this.worldObj)) {
    ...
    private int
    userPower(final int ticksPassed, final int bonusValue, final double acceleratorTax) {
        try {
            return (int
    ...
    private ItemStack pushTo(ItemStack output, final ForgeDirection d) {
        ...
        final TileEntity te = this.getWorldObj().getTileEntity(
    ...
    private void updatePowerState() {
        ...
        try {
            newState = this.getProxy().isActive()
                && this.getProxy().getEnergy().extractAEPower(
                       1, Actionable.SIMULATE, PowerMultiplier.CONFIG
                   ) > 0.0001;
