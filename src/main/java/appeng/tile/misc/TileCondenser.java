...
public class TileCondenser extends AEBaseInvTile
    implements IFluidHandler, IConfigManagerHost, IConfigurableObject {
    ...
    public void addPower(final double rawPower) {
        ...
        this.setStoredPower(
            Math.max(0.0, Math.min(this.getStorage(), this.getStoredPower()))
        );

        final double requiredPower = this.getRequiredPower();
        final ItemStack output = this.getOutput();
        while (requiredPower <= this.getStoredPower() && output != null
               && requiredPower > 0) {
    ...
    private boolean canAddOutput(final ItemStack output) {
        ...
        return outputStack == null
            || (Platform.isSameItem(outputStack, output)
