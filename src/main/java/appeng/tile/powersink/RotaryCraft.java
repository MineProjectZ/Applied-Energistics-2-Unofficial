...
public abstract class RotaryCraft extends IC2 implements AdvancedShaftPowerReceiver, Transducerable
{
    ...
    @Override
    public final boolean addPower(
        final int torque, final int omega, final long power, final ForgeDirection side
    ) {
        this.omega = omega;
        this.torque = torque;
        this.power = power;
    ...
    public final ArrayList<String> getMessages(
        ...
        if (this.power >= 1000000000) {
            out = String.format(
                "Receiving %.3f GW @ %d rad/s.", this.power / 1000000000.0D, this.omega
            );
        } else if (this.power >= 1000000) {
            out = String.format(
                "Receiving %.3f MW @ %d rad/s.", this.power / 1000000.0D, this.omega
            );
        } else if (this.power >= 1000) {
            out = String.format(
                "Receiving %.3f kW @ %d rad/s.", this.power / 1000.0D, this.omega
            );
        } else {
