...
public abstract class IC2 extends AERootPoweredTile implements IEnergySink {
    ...
    @Override
    public final double injectEnergy(
        ...
        final double overflow = PowerUnits.EU.convertTo(
            PowerUnits.AE, this.injectExternalPower(PowerUnits.EU, amount)
        );
    ...
    private void removeFromENet() {
        if (IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.IC2)) {
            final IIC2 ic2Integration
                = (IIC2) IntegrationRegistry.INSTANCE.getInstance(IntegrationType.IC2);
