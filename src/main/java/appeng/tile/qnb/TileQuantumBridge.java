...
public class TileQuantumBridge extends AENetworkInvTile implements IAEMultiBlock {
    ...
    private boolean isCenter() {
        for (final Block link :
             AEApi.instance().definitions().blocks().quantumLink().maybeBlock().asSet()) {
    ...
    public void onReady() {
        ...
        final boolean isPresent
            = maybeLinkBlock.isPresent() && maybeLinkStack.isPresent();
    ...
    public void
    updateStatus(final QuantumCluster c, final byte flags, final boolean affectWorld) {
        this.cluster = c;
    ...
    public boolean isCorner() {
        return (this.constructed & this.getCorner()) == this.getCorner()
            && this.constructed != -1;
    }
    ...
    public boolean isPowered() {
        if (Platform.isClient()) {
            return (this.constructed & this.powered) == this.powered
                && this.constructed != -1;
        }
