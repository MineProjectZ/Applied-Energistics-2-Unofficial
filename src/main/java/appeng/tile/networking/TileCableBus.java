...
public class TileCableBus extends AEBaseTile implements AEMultiTile, ICustomCollision {
    ...
    protected void updateTileSetting() {
        if (this.getCableBus().isRequiresDynamicRender()) {
            try {
                ...
                tcb.copyFrom(this);
                this.getWorldObj().setTileEntity(
                    this.xCoord, this.yCoord, this.zCoord, tcb
                );
    ...
    @Override
    public void
    getDrops(final World w, final int x, final int y, final int z, final List drops) {
    ...
    public void onReady() {
        ...
        if (this.getCableBus().isEmpty()) {
            if (this.worldObj.getTileEntity(this.xCoord, this.yCoord, this.zCoord)
                == this) {
    ...
    addPart(final ItemStack is, final ForgeDirection side, final EntityPlayer player) {
    ...
    public Iterable<AxisAlignedBB> getSelectedBoundingBoxesFromPool(
        ...
        return this.getCableBus().getSelectedBoundingBoxesFromPool(
            false, true, e, visual
        );
    }
    ...
    public void cleanup() {
        if (IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.ImmibisMicroblocks)) {
            final IImmibisMicroblocks imb = (IImmibisMicroblocks
        ...
        this.getWorldObj().setBlock(
            this.xCoord, this.yCoord, this.zCoord, Platform.AIR_BLOCK
        );
    }
    ...
    public void addCollidingBlockToList(
        ...
        for (final AxisAlignedBB bx :
             this.getSelectedBoundingBoxesFromPool(w, x, y, z, e, false)) {
            out.add(AxisAlignedBB.getBoundingBox(
                bx.minX, bx.minY, bx.minZ, bx.maxX, bx.maxY, bx.maxZ
            ));
        }
    }
