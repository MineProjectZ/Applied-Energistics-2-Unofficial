...
public final class AEItemStack
    extends AEStack<IAEItemStack> implements IAEItemStack, Comparable<AEItemStack> {
    ...
    private AEItemStack(final ItemStack is) {
        if (is == null) {
            throw new InvalidParameterException(
                "null is not a valid ItemStack for AEItemStack."
            );
        }
        ...
        if (item == null) {
            throw new InvalidParameterException(
                "Contained item is null, thus not a valid ItemStack for AEItemStack."
            );
        }
        ...
        if (this.getDefinition().getItem() == null) {
            throw new InvalidParameterException(
                "This ItemStack is bad, it has a null item."
            );
        }

        /*
         * Prevent an Item from changing the damage value on me... Either, this or a core
         * mod.
         */

        /*
         * Super hackery.
         * is.itemID = appeng.api.Materials.matQuartz.itemID; damageValue =
         * is.getItemDamage(); is.itemID = itemID;
         */
        ...
        if (tagCompound != null) {
            this.getDefinition().setTagCompound((AESharedNBT
            ) AESharedNBT.getSharedTagCompound(tagCompound, is));
        }
    ...
    public static IAEItemStack loadItemStackFromPacket(final ByteBuf data)
        throws IOException {
    ...
    public void add(final IAEItemStack option) {
        ...
        this.setCountRequestable(
            this.getCountRequestable() + option.getCountRequestable()
        );
    ...
    public void writeToNBT(final NBTTagCompound i) {
        /*
         * Mojang Fucked this over ; GC Optimization - Ugly Yes, but it saves a lot in the
         * memory department.
         */

        /*
         * NBTBase id = i.getTag( "id" ); NBTBase Count = i.getTag( "Count" ); NBTBase Cnt
         * = i.getTag( "Cnt" ); NBTBase Req = i.getTag( "Req" ); NBTBase Craft = i.getTag(
         * "Craft" ); NBTBase Damage = i.getTag( "Damage" );
         */

        /*
         * if ( id != null && id instanceof NBTTagShort ) ((NBTTagShort) id).data =
         * (short) this.def.item.itemID; else
         */
        i.setShort(
            "id", (short) Item.itemRegistry.getIDForObject(this.getDefinition().getItem())
        );

        /*
         * if ( Count != null && Count instanceof NBTTagByte ) ((NBTTagByte) Count).data =
         * (byte) 0; else
         */
        i.setByte("Count", (byte) 0);

        /*
         * if ( Cnt != null && Cnt instanceof NBTTagLong ) ((NBTTagLong) Cnt).data =
         * this.stackSize; else
         */
        i.setLong("Cnt", this.getStackSize());

        /*
         * if ( Req != null && Req instanceof NBTTagLong ) ((NBTTagLong) Req).data =
         * this.stackSize; else
         */
        i.setLong("Req", this.getCountRequestable());

        /*
         * if ( Craft != null && Craft instanceof NBTTagByte ) ((NBTTagByte) Craft).data =
         * (byte) (this.isCraftable() ? 1 : 0); else
         */
        i.setBoolean("Craft", this.isCraftable());

        /*
         * if ( Damage != null && Damage instanceof NBTTagShort ) ((NBTTagShort)
         * Damage).data = (short) this.def.damageValue; else
         */
                    ...
                    try {
                        if (mode == FuzzyMode.IGNORE_ALL) {
                            ...
                            final float percentDamageOfA = 1.0f
                                - (float) a.getItemDamageForDisplay()
                                    / (float) a.getMaxDamage();
                            final float percentDamageOfB = 1.0f
                                - (float) b.getItemDamageForDisplay()
                                    / (float) b.getMaxDamage();

                            return (percentDamageOfA > mode.breakPoint)
                                == (percentDamageOfB > mode.breakPoint);
                        }
                ...
                if (this.getDefinition().getItem().isDamageable()) {
                    ...
                    try {
                        if (mode == FuzzyMode.IGNORE_ALL) {
                            ...
                            return (a.getItemDamageForDisplay() > 1)
                                == (o.getItemDamageForDisplay() > 1);
                        } else {
                            final float percentDamageOfA = 1.0f
                                - (float) a.getItemDamageForDisplay()
                                    / (float) a.getMaxDamage();
                            final float percentDamageOfB = 1.0f
                                - (float) o.getItemDamageForDisplay()
                        ...
                        if (mode == FuzzyMode.IGNORE_ALL) {
                            ...
                            final float percentDamageOfA
                                = (float) a.getItemDamage() / (float) a.getMaxDamage();
                            final float percentDamageOfB
                                = (float) o.getItemDamage() / (float) o.getMaxDamage();
    ...
    public boolean equals(final Object ia) {
        if (ia instanceof AEItemStack) {
            return ((AEItemStack) ia)
                .getDefinition()
                .equals(this.getDefinition()); // && def.tagCompound ==
            // ((AEItemStack)
            // ia).def.tagCompound;
    ...
    public int compareTo(final AEItemStack b) {
        ...
        final int displayDamage = this.getDefinition().getDisplayDamage()
            - b.getDefinition().getDisplayDamage();
        if (displayDamage != 0) {
    ...
    public String getDisplayName() {
        if (this.getDefinition().getDisplayName() == null) {
            this.getDefinition().setDisplayName(
                Platform.getItemDisplayName(this.getItemStack())
            );
        }
    ...
    public String getModID() {
        ...
        return this.getModName(this.getDefinition().setUniqueID(
            GameRegistry.findUniqueIdentifierFor(this.getDefinition().getItem())
        ));
    }
        ...
        if (newDef.getItem().isDamageable()) {
            if (fuzzy == FuzzyMode.IGNORE_ALL) {
                ...
                newDef.setDisplayDamage(
                    breakpoint <= this.getDefinition().getDisplayDamage() ? breakpoint : 0
                );
            }
