...
public class AESharedNBT extends NBTTagCompound implements IAETagCompound {
    ...
    private static final
        WeakHashMap<SharedSearchObject, WeakReference<SharedSearchObject>>
            SHARED_TAG_COMPOUND
     ...
     */
    static synchronized NBTTagCompound
    getSharedTagCompound(final NBTTagCompound tagCompound, final ItemStack s) {
    ...
    /*
     * returns true if the compound is part of the shared compound system ( and can thus
    ...
    private static AESharedNBT
    createFromCompound(final Item itemID, final int damageValue, final NBTTagCompound c) {
        ...
        x.comp
            = AEApi.instance().registries().specialComparison().getSpecialComparison(isc);
