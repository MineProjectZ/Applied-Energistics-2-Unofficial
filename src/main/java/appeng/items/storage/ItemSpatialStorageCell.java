...
public class ItemSpatialStorageCell extends AEBaseItem implements ISpatialStorageCell {
    ...
    public void addCheckedInformation(
        ...
        if (wc.x > 0) {
            lines.add(
                GuiText.StoredSize.getLocal() + ": " + wc.x + " x " + wc.y + " x " + wc.z
            );
        }
    }
        ...
        if (is.hasTagCompound()) {
            ...
            if (Platform.isServer()) {
                ...
                return new WorldCoord(
                    c.getInteger("sizeX"), c.getInteger("sizeY"), c.getInteger("sizeZ")
                );
            }
        }
    ...
    public WorldCoord getMin(final ItemStack is) {
        ...
        if (w != null) {
            final NBTTagCompound info
                = (NBTTagCompound) w.getWorldInfo().getAdditionalProperty("storageCell");
            if (info != null) {
                return new WorldCoord(
                    info.getInteger("minX"),
                    info.getInteger("minY"),
                    info.getInteger("minZ")
                );
            }
        }
        return new WorldCoord(0, 0, 0);
    ...
    public WorldCoord getMax(final ItemStack is) {
        ...
        if (w != null) {
            ...
            if (info != null) {
                return new WorldCoord(
                    info.getInteger("maxX"),
                    info.getInteger("maxY"),
                    info.getInteger("maxZ")
                );
            }
        }
        return new WorldCoord(0, 0, 0);
    ...
    public TransitionResult doSpatialTransition(
        final ItemStack is,
        final World w,
        final WorldCoord min,
        final WorldCoord max,
    ...
    private void setStoredSize(
        final ItemStack is, final int targetX, final int targetY, final int targetZ
    ) {
        if (is.hasTagCompound()) {
            final NBTTagCompound c = is.getTagCompound();
            final int dim = c.getInteger("StorageDim");
            ...
            WorldData.instance().dimensionData().setStoredSize(
                dim, targetX, targetY, targetZ
            );
        }
    }
}
