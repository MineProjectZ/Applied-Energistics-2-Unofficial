...
public abstract class AERootPoweredItem
    extends AEBaseItem implements IAEItemPowerStorage {
    ...
    private double getInternalBattery(
        final ItemStack is, final batteryOperation op, final double adjustment
    ) {
    ...
    double injectExternalPower(
        final PowerUnits input,
        final ItemStack is,
        final double amount,
        final boolean simulate
    ) {
        if (simulate) {
            final int requiredEU = (int) PowerUnits.AE.convertTo(
                PowerUnits.EU, this.getAEMaxPower(is) - this.getAECurrentPower(is)
            );
            ...
            final double powerRemainder
                = this.injectAEPower(is, PowerUnits.EU.convertTo(PowerUnits.AE, amount));
