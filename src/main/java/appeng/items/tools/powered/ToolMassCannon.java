...
public class ToolMassCannon extends AEBasePoweredItem implements IStorageCell {
    ...
    public void postInit() {
        ...
        BlockDispenser.dispenseBehaviorRegistry.putObject(
            this, new DispenserMatterCannon()
        );
    }
    ...
    public void addCheckedInformation(
        ...
        if (cdi instanceof CellInventoryHandler) {
            ...
            if (cd != null) {
                lines.add(
                    cd.getUsedBytes() + " " + GuiText.Of.getLocal() + ' '
                    + cd.getTotalBytes() + ' ' + GuiText.BytesUsed.getLocal()
                );
                lines.add(
                    cd.getStoredItemTypes() + " " + GuiText.Of.getLocal() + ' '
                    + cd.getTotalItemTypes() + ' ' + GuiText.Types.getLocal()
                );
            }
        }
    }
    ...
    @Override
    public ItemStack
    onItemRightClick(final ItemStack item, final World w, final EntityPlayer p) {
            ...
            if (inv != null) {
                ...
                if (aeAmmo instanceof IAEItemStack) {
                    ...
                    for (int sh = 0; sh < shots; sh++) {
                        ...
                        ammo.stackSize = 1;
                        aeAmmo = inv.extractItems(
                            aeAmmo, Actionable.MODULATE, new PlayerSource(p, null)
                        );
                        if (aeAmmo == null) {
                            return item;
                        }
                        ...
                        final float f = 1.0F;
                        final float f1 = p.prevRotationPitch
                            + (p.rotationPitch - p.prevRotationPitch) * f;
                        final float f2
                            = p.prevRotationYaw + (p.rotationYaw - p.prevRotationYaw) * f;
                        final double d0 = p.prevPosX + (p.posX - p.prevPosX) * f;
                        final double d1
                            = p.prevPosY + (p.posY - p.prevPosY) * f + 1.62D - p.yOffset;
                        final double d2 = p.prevPosZ + (p.posZ - p.prevPosZ) * f;
                        final Vec3 vec3 = Vec3.createVectorHelper(d0, d1, d2);
                        final float f3
                            = MathHelper.cos(-f2 * 0.017453292F - (float) Math.PI);
                        final float f4
                            = MathHelper.sin(-f2 * 0.017453292F - (float) Math.PI);
                        ...
                        final Vec3 direction
                            = Vec3.createVectorHelper(f7 * d3, f6 * d3, f8 * d3);
                        direction.normalize();

                        final float penetration
                            = AEApi.instance().registries().matterCannon().getPenetration(
                                ammo
                            ); // 196.96655f;
                        if (penetration <= 0) {
                            ...
                            if (type.getItem() instanceof ItemPaintBall) {
                                this.shootPaintBalls(
                                    type, w, p, vec3, vec31, direction, d0, d1, d2
                                );
                            }
                            return item;
                        } else {
                            this.standardAmmo(
                                penetration, w, p, vec3, vec31, direction, d0, d1, d2
                            );
                        }
                    }
                } else {
        ...
        while (penetration > 0 && hasDestroyed) {
            ...
            if (pos != null) {
                ...
                if (pos.typeOfHit == MovingObjectType.ENTITY) {
                    ...
                    if (!AEConfig.instance.isFeatureEnabled(
                            AEFeature.MassCannonBlockDamage
                        )) {
                        penetration = 0;
                    } else {
                        ...
                        final float hardness
                            = b.getBlockHardness(w, pos.blockX, pos.blockY, pos.blockZ)
                            * 9.0f;
                        if (hardness >= 0.0) {
                            if (penetration > hardness
                                && Platform.hasPermissions(
                                    new DimensionalCoord(
                                        w, pos.blockX, pos.blockY, pos.blockZ
                                    ),
                                    p
                                )) {
                                hasDestroyed = true;
                                ...
                                // w.destroyBlock( pos.blockX, pos.blockY, pos.blockZ,
                                // true );
                            }
                        }
                    }
    ...
    @Override
    public boolean
    isBlackListed(final ItemStack cellItem, final IAEItemStack requestedAddition) {
        final float pen = AEApi.instance().registries().matterCannon().getPenetration(
            requestedAddition.getItemStack()
        );
        if (pen > 0) {
            return false;
        }
