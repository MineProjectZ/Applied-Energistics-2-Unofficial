...
public class ToolChargedStaff extends AEBasePoweredItem {
    ...
    public boolean hitEntity(
        ...
        if (hitter instanceof EntityPlayer
            && ForgeEventFactory.onItemUseStart((EntityPlayer) hitter, item, 1) <= 0)
            return false;
        ...
        if (this.getAECurrentPower(item) > 300) {
            ...
            if (Platform.isServer()) {
                for (int x = 0; x < 2; x++) {
                    final float dx = (float
                    ) (Platform.getRandomFloat() * target.width + target.boundingBox.minX
                    );
                    final float dy = (float
                    ) (Platform.getRandomFloat() * target.height + target.boundingBox.minY
                    );
                    final float dz = (float
                    ) (Platform.getRandomFloat() * target.width + target.boundingBox.minZ
                    );
