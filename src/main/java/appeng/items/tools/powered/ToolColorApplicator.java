...
public class ToolColorApplicator extends AEBasePoweredItem
    ...
    private static final Map<Integer, AEColor> ORE_TO_COLOR
        = new HashMap<Integer, AEColor>();
    ...
    public ToolColorApplicator() {
        ...
        if (Platform.isClient()) {
            MinecraftForgeClient.registerItemRenderer(
                this, new ToolColorApplicatorRender()
            );
        }
    }
    ...
    public boolean onItemUse(
        ...
        if (inv != null) {
            final IAEItemStack option = inv.extractItems(
                AEItemStack.create(paintBall), Actionable.SIMULATE, new BaseActionSource()
            );
            ...
            if (paintBall != null && paintBall.getItem() instanceof ItemSnowball) {
                ...
                if (te instanceof IColorableTile) {
                    if (this.getAECurrentPower(is) > powerPerUse
                        ...
                        if (((IColorableTile) te)
                                .recolourBlock(orientation, AEColor.Transparent, p)) {
                            inv.extractItems(
                ...
                // clean paint balls..
                final Block testBlk = w.getBlock(
                    x + orientation.offsetX,
                    y + orientation.offsetY,
                    z + orientation.offsetZ
                );
                final TileEntity painted = w.getTileEntity(
                    x + orientation.offsetX,
                    y + orientation.offsetY,
                    z + orientation.offsetZ
                );
                if (this.getAECurrentPower(is) > powerPerUse
                    && testBlk instanceof BlockPaint && painted instanceof TilePaint) {
                    inv.extractItems(
                        AEItemStack.create(paintBall),
                        Actionable.MODULATE,
                        new BaseActionSource()
    ...
    private ItemStack
    findNextColor(final ItemStack is, final ItemStack anchor, final int scrollOffset) {
    ...
    private boolean recolourBlock(
        ...
        if (blk == Blocks.hardened_clay) {
            return w.setBlock(
                x, y, z, Blocks.stained_hardened_clay, newColor.ordinal(), 3
            );
        }
        ...
        if (blk instanceof BlockCableBus) {
            return ((BlockCableBus) blk)
                .recolourBlock(w, x, y, z, side, newColor.ordinal(), p);
        }
    ...
    isBlackListed(final ItemStack cellItem, final IAEItemStack requestedAddition) {
        if (requestedAddition != null) {
            ...
            return !(
                requestedAddition.getItem() instanceof ItemPaintBall
                && requestedAddition.getItemDamage() < 20
            );
        }
        return true;
    }
    ...
    @Override
    public String
    getUnlocalizedGroupName(final Set<ItemStack> others, final ItemStack is) {
