...
public class ToolBiometricCard extends AEBaseItem implements IBiometricCard {
    public ToolBiometricCard() {
        ...
        if (Platform.isClient()) {
            MinecraftForgeClient.registerItemRenderer(
                this, new ToolBiometricCardRender()
            );
        }
    }
    ...
    @Override
    public ItemStack
    onItemRightClick(final ItemStack is, final World w, final EntityPlayer p) {
    ...
    public EnumSet<SecurityPermissions> getPermissions(final ItemStack is) {
        final NBTTagCompound tag = Platform.openNbtData(is);
        final EnumSet<SecurityPermissions> result
            = EnumSet.noneOf(SecurityPermissions.class);
    ...
    @Override
    public boolean
    hasPermission(final ItemStack is, final SecurityPermissions permission) {
        final NBTTagCompound tag = Platform.openNbtData(is);
    ...
    @Override
    public void
    removePermission(final ItemStack itemStack, final SecurityPermissions permission) {
        final NBTTagCompound tag = Platform.openNbtData(itemStack);
    ...
    @Override
    public void
    addPermission(final ItemStack itemStack, final SecurityPermissions permission) {
        final NBTTagCompound tag = Platform.openNbtData(itemStack);
    ...
    public void registerPermissions(
        final ISecurityRegistry register, final IPlayerRegistry pr, final ItemStack is
    ) {
