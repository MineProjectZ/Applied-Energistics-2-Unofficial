...
public class ToolNetworkTool
    extends AEBaseItem implements IGuiItem, IAEWrench, IToolWrench {
    ...
    public IGuiItemObject getGuiObject(
        final ItemStack is, final World world, final int x, final int y, final int z
    ) {
        final TileEntity te = world.getTileEntity(x, y, z);
        return new NetworkToolViewer(
            is, (IGridHost) (te instanceof IGridHost ? te : null)
        );
    }

    @Override
    public ItemStack
    onItemRightClick(final ItemStack it, final World w, final EntityPlayer p) {
    ...
    public boolean onItemUseFirst(
        ...
        final MovingObjectPosition mop = new MovingObjectPosition(
            x, y, z, side, Vec3.createVectorHelper(hitX, hitY, hitZ)
        );
        final TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof IPartHost) {
            ...
            if (part.part != null || part.facade != null) {
                if (part.part instanceof INetworkToolAgent
                    && !((INetworkToolAgent) part.part).showNetworkInfo(mop)) {
                    return false;
                }
            }
        ...
        if (Platform.isClient()) {
            NetworkHandler.instance.sendToServer(
                new PacketClick(x, y, z, side, hitX, hitY, hitZ)
            );
        }
        return true;
    }
            ...
            if (!p.isSneaking()) {
                ...
                if (te instanceof IGridHost) {
                    Platform.openGUI(
                        ...
                        GuiBridge.GUI_NETWORK_STATUS
                    );
                } else {
                    Platform.openGUI(
                        p, null, ForgeDirection.UNKNOWN, GuiBridge.GUI_NETWORK_TOOL
                    );
                }
