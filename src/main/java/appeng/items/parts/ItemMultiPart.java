...
public final class ItemMultiPart extends AEBaseItem implements IPartItem, IItemGroup {
    ...
    private static final Comparator<Entry<Integer, PartTypeWithVariant>>
    ...
    public ItemMultiPart(final IPartHelper partHelper) {
        ...
        this.registered
            = new HashMap<Integer, PartTypeWithVariant>(INITIAL_REGISTERED_CAPACITY);
    ...
    private void processMetaOverlap(
        ...
        if (registeredPartType != null) {
            throw new IllegalStateException(
                "Meta Overlap detected with type " + mat + " and damage " + partDamage
    ...
    public String getItemStackDisplayName(final ItemStack is) {
        ...
        if (pt.isCable()) {
            ...
            final PartTypeWithVariant registeredPartType
                = this.registered.get(itemDamage);
            if (registeredPartType != null) {
                return super.getItemStackDisplayName(is) + " - "
                    + variants[registeredPartType.variant].toString();
            }
        }

        if (pt.getExtraName() != null) {
            return super.getItemStackDisplayName(is) + " - "
                + pt.getExtraName().getLocal();
        }
    ...
    public void registerIcons(final IIconRegister iconRegister) {
        for (final Entry<Integer, PartTypeWithVariant> part :
             this.registered.entrySet()) {
            final String tex = "appliedenergistics2:"
                + this.getName(new ItemStack(this, 1, part.getKey()));
    ...
    private static final class PartTypeWithVariant {
        ...
        public String toString() {
            return "PartTypeWithVariant{"
                + "part=" + this.part + ", variant=" + this.variant + ", ico=" + this.ico
                + '}';
        }
    }

    private static final class RegisteredComparator
        implements Comparator<Entry<Integer, PartTypeWithVariant>> {
        @Override
        public int compare(
            final Entry<Integer, PartTypeWithVariant> o1,
            final Entry<Integer, PartTypeWithVariant> o2
        ) {
