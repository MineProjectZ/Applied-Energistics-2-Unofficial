...
public final class ItemMultiMaterial
    extends AEBaseItem implements IStorageComponent, IUpgradeModule {
    public static ItemMultiMaterial instance;

    private static final int KILO_SCALAR = 1024;

    private final Map<Integer, MaterialType> dmgToMaterial
        = new HashMap<Integer, MaterialType>();
        ...
        if (u != null) {
            ...
            for (final Entry<ItemStack, Integer> j : u.getSupported().entrySet()) {
                ...
                if (j.getKey().getItem() instanceof IItemGroup) {
                    ...
                    final String str = ig.getUnlocalizedGroupName(
                        u.getSupported().keySet(), j.getKey()
                    );
                    if (str != null) {
                        name = Platform.gui_localize(str)
                            + (limit > 1 ? " (" + limit + ')' : "");
                    }
                }

                if (name == null) {
                    name = j.getKey().getDisplayName()
                        + (limit > 1 ? " (" + limit + ')' : "");
                }
    ...
    public IStackSrc createMaterial(final MaterialType mat) {
        Preconditions.checkState(
            !mat.isRegistered(), "Cannot create the same material twice."
        );
                ...
                if (u != null) {
                    ...
                    if (ad != null) {
                        ...
                        player.inventory.setInventorySlotContents(
                            player.inventory.currentItem, ad.addItems(is)
                        );
                        return true;
                    }
                }
    ...
    @Override
    public Entity
    createEntity(final World w, final Entity location, final ItemStack itemstack) {
        ...
        try {
            eqi = droppedEntity
                      ...
                      .newInstance(
                          w, location.posX, location.posY, location.posZ, itemstack
                      );
        ...
        if (location instanceof EntityItem && eqi instanceof EntityItem) {
            ((EntityItem) eqi).delayBeforeCanPickup
