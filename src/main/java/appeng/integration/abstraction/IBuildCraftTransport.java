...
public interface IBuildCraftTransport {
     ...
     * @return {@code true} if the checked item is a {@link
    ...
    @Nullable
    IFacadePart
    createFacadePart(@Nullable Block blk, int meta, @Nonnull ForgeDirection side);

    /**
     * @param held create facade for that item
     * @param side on which side should the part be rendered, should rather be not {@code
     *     null}
     * @return new instance using the {@code held} and side as direct argument, no logic
     *     in between
     ...
     * @return item with the block and metadata based on the facade or {@code null} if
     *     {@code facade} was not a facade
    ...
    /**
     * @return texture of buildcraft cobblestone structure pipe or null if something bad
     *     happens
     */
    @Nullable
     ...
     *
     * @param te  preferred something like a buildcraft injectable, can handle anything,
     *     just fails that way
