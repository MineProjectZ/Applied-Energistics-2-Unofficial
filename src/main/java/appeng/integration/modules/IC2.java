...
public class IC2 implements IIC2, IIntegrationModule {
    ...
    public IC2() {
        ...
        IntegrationHelper
            .testClassExistence(this, ic2.api.recipe.RecipeInputItemStack.class);
    }
    ...
    public void init() {
        ...
        if (IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.IC2)) {
            partHelper.registerNewLayer(
                "appeng.parts.layers.LayerIEnergySink", "ic2.api.energy.tile.IEnergySink"
            );
            partHelper.registerNewLayer(
                "appeng.parts.layers.LayerIEnergySource",
    ...
    public void postInit() {
        ...
        reg.addNewAttunement(
            this.getItem("insulatedCopperCableItem"), TunnelType.IC2_POWER
        );
        reg.addNewAttunement(this.getItem("goldCableItem"), TunnelType.IC2_POWER);
        reg.addNewAttunement(
            this.getItem("insulatedGoldCableItem"), TunnelType.IC2_POWER
        );
        reg.addNewAttunement(this.getItem("ironCableItem"), TunnelType.IC2_POWER);
        reg.addNewAttunement(
            this.getItem("insulatedIronCableItem"), TunnelType.IC2_POWER
        );
        ...
        // this is gone?
        // AEApi.INSTANCE().registries().matterCannon().registerAmmo( getItem(
        // "uraniumDrop" ), 238.0289 );
    }

    @Override
    public void addToEnergyNet(final TileEntity appEngTile) {
        MinecraftForge.EVENT_BUS.post(
            new ic2.api.energy.event.EnergyTileLoadEvent((IEnergyTile) appEngTile)
        );
    }
    ...
    public void removeFromEnergyNet(final TileEntity appEngTile) {
        MinecraftForge.EVENT_BUS.post(
            new ic2.api.energy.event.EnergyTileUnloadEvent((IEnergyTile) appEngTile)
        );
    }
    ...
    public void maceratorRecipe(final ItemStack in, final ItemStack out) {
        ic2.api.recipe.Recipes.macerator.addRecipe(
            new RecipeInputItemStack(in, in.stackSize), null, out
        );
    }
}
