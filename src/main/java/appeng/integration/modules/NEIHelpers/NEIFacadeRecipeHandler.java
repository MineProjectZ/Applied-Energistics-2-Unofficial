...
public class NEIFacadeRecipeHandler extends TemplateRecipeHandler {
    ...
    public void loadTransferRects() {
        this.transferRects.add(new TemplateRecipeHandler.RecipeTransferRect(
            new Rectangle(84, 23, 24, 18), "crafting"
        ));
    }
    ...
    public void loadCraftingRecipes(final String outputId, final Object... results) {
        if ((outputId.equals("crafting"))
            && (this.getClass() == NEIFacadeRecipeHandler.class)) {
            ...
            for (final ItemStack anchorStack :
                ...
                for (final ItemStack is : facades) {
                    final CachedShapedRecipe recipe
                        = new CachedShapedRecipe(this.facade, anchorStack, is);
    ...
    public void loadCraftingRecipes(final ItemStack result) {
        if (result.getItem() == this.facade) {
            for (final ItemStack anchorStack :
                 this.anchorDefinition.maybeStack(1).asSet()) {
                final CachedShapedRecipe recipe
                    = new CachedShapedRecipe(this.facade, anchorStack, result);
                recipe.computeVisuals();
                this.arecipes.add(recipe);
            }
        }
    ...
    private final class CachedShapedRecipe extends TemplateRecipeHandler.CachedRecipe {
        ...
        public CachedShapedRecipe(
            final IFacadeItem facade, final ItemStack anchor, final ItemStack output
        ) {
            output.stackSize = 4;
            ...
            this.setIngredients(
                ...
                3,
                new Object[] {
                    null, anchor, null, anchor, in, anchor, null, anchor, null }
            );
        }

        public void
        setIngredients(final int width, final int height, final Object[] items) {
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    if (items[(y * width + x)] != null) {
                        final ItemStack is = (ItemStack) items[(y * width + x)];
                        final PositionedStack stack
                            = new PositionedStack(is, 25 + x * 18, 6 + y * 18, false);
                        stack.setMaxSize(1);
        ...
        public List<PositionedStack> getIngredients() {
            return this.getCycledIngredients(
                NEIFacadeRecipeHandler.this.cycleticks / 20, this.ingredients
            );
        }
