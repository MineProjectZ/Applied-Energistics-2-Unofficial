...
public final class RF implements IIntegrationModule {
    ...
    public RF() {
        ...
        IntegrationHelper
            .testClassExistence(this, cofh.api.energy.IEnergyConnection.class);
    }
    ...
    public void init() {
        ...
        if (IntegrationRegistry.INSTANCE.isEnabled(IntegrationType.RF)) {
            partHelper.registerNewLayer(
                "appeng.parts.layers.LayerIEnergyHandler",
    ...
    public void postInit() {
        ...
        this.registerRFAttunement(
            "ExtraUtilities", "generator", OreDictionary.WILDCARD_VALUE
        );

        this.registerRFAttunement(
            "ThermalExpansion", "Cell", OreDictionary.WILDCARD_VALUE
        );
        this.registerRFAttunement(
            "ThermalExpansion", "Dynamo", OreDictionary.WILDCARD_VALUE
        );
        ...
        this.registerRFAttunement(
            "EnderIO", "itemPowerConduit", OreDictionary.WILDCARD_VALUE
        );
    ...
    private void
    registerRFAttunement(final String mod, final String name, final int dmg) {
        assert mod != null;
        assert !mod.isEmpty();
        assert name != null;
        assert !name.isEmpty();
        ...
        if (modItem != null) {
            ...
            AEApi.instance().registries().p2pTunnel().addNewAttunement(
                modItem, TunnelType.RF_POWER
            );
        }
    }
}
