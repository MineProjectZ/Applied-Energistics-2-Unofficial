...
public class FactorizationBarrel implements IMEInventory<IAEItemStack> {
    ...
    public long remainingItemCount() {
        return this.fProxy.barrelGetMaxItemCount(this.te)
            - this.fProxy.barrelGetItemCount(this.te);
    }
    ...
    public IAEItemStack injectItems(
        ...
        if (this.containsItemType(input, mode == Actionable.SIMULATE)) {
            ...
            final int newTotal
                = (int) this.storedItemCount() + (int) input.getStackSize();
            if (newTotal > max) {
                if (mode == Actionable.MODULATE) {
            ...
            if (request.getStackSize() >= howMany) {
                ...
                if (mode == Actionable.MODULATE) {
                    this.fProxy.barrelSetCount(
                        this.te, (int) (howMany - request.getStackSize())
                    );
                }
                return request.copy();
            }
