...
public class BSCrate implements IMEInventory<IAEItemStack> {
    ...
    public IAEItemStack extractItems(
        ...
        if (mode == Actionable.SIMULATE) {
            ...
            return howMany > request.getStackSize()
                ? request
                : request.copy().setStackSize(howMany);
        }

        final ItemStack obtained = this.crateStorage.extractItems(
            request.getItemStack(), (int) request.getStackSize()
        );
