...
public final class CraftGuide
    extends CraftGuideAPIObject implements IIntegrationModule, RecipeProvider {
    ...
    private static final Slot[] GRINDER_SLOTS = {
        ...
        new ItemSlot(41, 21, SLOT_SIZE, SLOT_SIZE, true)
            .drawOwnBackground()
            .setSlotType(SlotType.OUTPUT_SLOT),
        new ChanceSlot(59, 12, SLOT_SIZE, SLOT_SIZE, true)
            .setRatio(GRINDER_RATIO)
            .setFormatString(" (%1$.2f%% chance)")
            .drawOwnBackground()
            .setSlotType(SlotType.OUTPUT_SLOT),
        new ChanceSlot(59, 30, SLOT_SIZE, SLOT_SIZE, true)
            .setRatio(GRINDER_RATIO)
            .setFormatString(" (%1$.2f%% chance)")
            .drawOwnBackground()
            .setSlotType(SlotType.OUTPUT_SLOT),
    ...
    private static final Slot[] INSCRIBER_SLOTS = {
        ...
        new ItemSlot(50, 21, SLOT_SIZE, SLOT_SIZE, true)
            .drawOwnBackground()
            .setSlotType(SlotType.OUTPUT_SLOT),
    ...
    private static final Slot[] SHAPELESS_CRAFTING_SLOTS = {
        ...
        new ItemSlot(59, 21, SLOT_SIZE, SLOT_SIZE, true)
            .setSlotType(SlotType.OUTPUT_SLOT),
    };
    ...
    public CraftGuide() {
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.CraftGuideLog.class);
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.DefaultRecipeTemplate.class);
        IntegrationHelper.testClassExistence(
            this, uristqwerty.CraftGuide.RecipeGeneratorImplementation.class
        );
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.api.ChanceSlot.class);
        IntegrationHelper.testClassExistence(
            this, uristqwerty.CraftGuide.api.CraftGuideAPIObject.class
        );
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.api.ItemSlot.class);
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.api.RecipeGenerator.class);
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.api.RecipeProvider.class);
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.api.RecipeTemplate.class);
        IntegrationHelper.testClassExistence(this, uristqwerty.CraftGuide.api.Slot.class);
        IntegrationHelper
            .testClassExistence(this, uristqwerty.CraftGuide.api.SlotType.class);
        IntegrationHelper.testClassExistence(
            this, uristqwerty.gui_craftguide.texture.DynamicTexture.class
        );
        IntegrationHelper.testClassExistence(
            this, uristqwerty.gui_craftguide.texture.TextureClip.class
        );
    }
    ...
    public void generateRecipes(final RecipeGenerator generator) {
        ...
        if (uristqwerty.CraftGuide.CraftGuide.newerBackgroundStyle) {
            craftingTemplate
                = generator.createRecipeTemplate(CRAFTING_SLOTS_OWN_BG, null);
            smallTemplate
                = generator.createRecipeTemplate(SMALL_CRAFTING_SLOTS_OWN_BG, null);
        } else {
            final TextureClip craftingBG = new TextureClip(
            ...
            final TextureClip craftingSelected = new TextureClip(
                DynamicTexture.instance("recipe_backgrounds"),
                82,
                1,
                TEXTURE_WIDTH,
            ...
            final TextureClip smallBG = new TextureClip(
                DynamicTexture.instance("recipe_backgrounds"),
                1,
                61,
                TEXTURE_WIDTH,
            ...
            final TextureClip smallSelected = new TextureClip(
                DynamicTexture.instance("recipe_backgrounds"),
                82,
                61,
                TEXTURE_WIDTH,
            ...
            smallTemplate = new DefaultRecipeTemplate(
                SMALL_CRAFTING_SLOTS,
                RecipeGeneratorImplementation.workbench,
                smallBG,
                smallSelected
            );
        ...
        final TextureClip shapelessBG = new TextureClip(
            DynamicTexture.instance("recipe_backgrounds"),
            1,
            121,
            TEXTURE_WIDTH,
        ...
        final TextureClip shapelessSelected = new TextureClip(
            DynamicTexture.instance("recipe_backgrounds"),
            82,
            121,
            TEXTURE_WIDTH,
        ...
        final RecipeTemplate shapelessTemplate = new DefaultRecipeTemplate(
            SHAPELESS_CRAFTING_SLOTS,
            RecipeGeneratorImplementation.workbench,
            shapelessBG,
            shapelessSelected
        );
        ...
        this.addCraftingRecipes(
            craftingTemplate, smallTemplate, shapelessTemplate, generator
        );
    ...
    private void addCraftingRecipes(
        ...
        final RecipeTemplate templateSmall,
        final RecipeTemplate templateShapeless,
        ...
        for (final IRecipe recipe : recipes) {
            try {
                ...
                if (errCount >= 5) {
                    CraftGuideLog.log(
                        "AppEng CraftGuide integration: Stack trace limit reached, further stack traces from this invocation will not be logged to the console. They will still be logged to (.minecraft)/config/CraftGuide/CraftGuide.log",
                        true
                    );
                } else {
                    e.printStackTrace();
    ...
    private void addGrinderRecipes(
        ...
        final ItemStack handle
            = api.definitions().blocks().crankHandle().maybeStack(1).orNull();
        final RecipeTemplate grinderTemplate
            = generator.createRecipeTemplate(GRINDER_SLOTS, grindstone);
        ...
        for (final IGrinderEntry recipe : api.registries().grinder().getRecipes()) {
            generator.addRecipe(
                ...
                new Object[] {
                    recipe.getInput(),
                    recipe.getOutput(),
                    new Object[] { recipe.getOptionalOutput(),
                                   (int) (recipe.getOptionalChance() * GRINDER_RATIO) },
                    new Object[] { recipe.getSecondOptionalOutput(),
                                   (int) (recipe.getOptionalChance() * GRINDER_RATIO) },
                    handle,
                    grindstone }
            );
    ...
    private void addInscriberRecipes(
        final IAppEngApi api, final ItemStack inscriber, final RecipeGenerator generator
    ) {
        final RecipeTemplate inscriberTemplate
            = generator.createRecipeTemplate(INSCRIBER_SLOTS, inscriber);

        for (final IInscriberRecipe recipe : api.registries().inscriber().getRecipes()) {
            generator.addRecipe(
                inscriberTemplate,
                new Object[] { recipe.getInputs(),
    ...
    private Object[] getCraftingShapelessRecipe(
        final List<?> items, final ItemStack recipeOutput
    ) {
    ...
    private Object[] getSmallShapedRecipe(
        final int width,
        final int height,
        final Object[] items,
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (output[i] instanceof IIngredient) {
                    try {
                        output[i]
                            = this.toCG(((IIngredient) output[i]).getItemStackSet());
                    } catch (final RegistrationError ignored) {
                    } catch (final MissingIngredientError ignored) {}
                }
            }
    ...
    private Object[] getCraftingShapedRecipe(
        final int width,
        final int height,
        final Object[] items,
        final ItemStack recipeOutput
    ...
    @Nullable
    private Object[] getCraftingRecipe(
        final IRecipe recipe, final boolean allowSmallGrid
    ) {
        if (recipe instanceof ShapelessRecipe) {
            final List<Object> items = ReflectionHelper.getPrivateValue(
                ShapelessRecipe.class, (ShapelessRecipe) recipe, "input"
            );

            return this.getCraftingShapelessRecipe(items, recipe.getRecipeOutput());
        } else if (recipe instanceof ShapedRecipe) {
            final int width = ReflectionHelper.getPrivateValue(
                ShapedRecipe.class, (ShapedRecipe) recipe, "width"
            );
            final int height = ReflectionHelper.getPrivateValue(
                ShapedRecipe.class, (ShapedRecipe) recipe, "height"
            );
            final Object[] items = ReflectionHelper.getPrivateValue(
                ShapedRecipe.class, (ShapedRecipe) recipe, "input"
            );

            if (allowSmallGrid && width < 3 && height < 3) {
                return this.getSmallShapedRecipe(
                    width, height, items, recipe.getRecipeOutput()
                );
            } else {
                return this.getCraftingShapedRecipe(
                    width, height, items, recipe.getRecipeOutput()
                );
            }
        }
