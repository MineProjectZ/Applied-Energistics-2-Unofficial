...
public final class ChannelWailaDataProvider extends BasePartWailaDataProvider {
    ...
    public List<String> getWailaBody(
        ...
        if (part instanceof PartCableSmart || part instanceof PartDenseCable) {
            ...
            final String formattedToolTip
                = String.format(WailaText.Channels.getLocal(), usedChannels, maxChannels);
     ...
     * else if the cache contains a previous stored value, this will be used. Default
     * value is 0.
     *
     ...
     */
    private byte getUsedChannels(
        final IPart part, final NBTTagCompound tag, final TObjectByteMap<IPart> cache
    ) {
     ...
     * <p/>
     * If the part is a cable, it writes the channel information in the {@code #tag} using
     * the {@code ID_USED_CHANNELS} key.
     *
