...
public final class PowerStorageWailaDataProvider extends BaseWailaDataProvider {
     ...
     * requested the new information from the server. <p/> The cache will be updated from
     * the server.
     */
    ...
    public List<String> getWailaBody(
        ...
        if (te instanceof IAEPowerStorage) {
            ...
            if (maxPower > 0) {
                ...
                final String formatCurrentPower
                    = Platform.formatPowerLong(internalCurrentPower, false);
                final String formatMaxPower
                    = Platform.formatPowerLong(internalMaxPower, false);

                currentToolTip.add(
                    WailaText.Contains.getLocal() + ": " + formatCurrentPower + " / "
                    + formatMaxPower
                );
            }
        }
     ...
     * <p/>
     * If the {@link net.minecraft.tileentity.TileEntity} is a {@link
     * appeng.api.networking.energy.IAEPowerStorage}, it writes the power information to
     * the {@code #tag} using the {@code #ID_CURRENT_POWER} key.
     *
        ...
        if (te instanceof IAEPowerStorage) {
            ...
            if (storage.getAEMaxPower() > 0) {
                final long internalCurrentPower
                    = (long) (100 * storage.getAECurrentPower());
     ...
     * <p/>
     * If the client received power information on the server, they are used, else if the
     * cache contains a previous stored value, this will be used. Default value is 0.
     *
