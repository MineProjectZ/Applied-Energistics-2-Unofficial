...
public enum IntegrationRegistry {
    ...
    public void add(final IntegrationType type) {
        if (type.side == IntegrationSide.CLIENT
            && FMLLaunchHandler.side() == Side.SERVER) {
            return;
        }

        if (type.side == IntegrationSide.SERVER
            && FMLLaunchHandler.side() == Side.CLIENT) {
            return;
        }

        this.modules.add(new IntegrationNode(
            type.dspName, type.modID, type, PACKAGE_PREFIX + type.name()
        ));
    }
    ...
    public Object getInstance(final IntegrationType name) {
        ...
        throw new IllegalStateException(
            "integration with " + name.name() + " is disabled."
        );
    }
}
