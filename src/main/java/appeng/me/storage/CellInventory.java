...
public class CellInventory implements ICellInventory {
    ...
    private CellInventory(final ItemStack o, final ISaveProvider container)
    ...
    public static IMEInventoryHandler<IAEItemStack>
    getCell(final ItemStack o, final ISaveProvider container2) {
        try {
    ...
    private static boolean isBlackListed(final IAEItemStack input) {
        if (BLACK_LIST.contains(
                (OreDictionary.WILDCARD_VALUE << Platform.DEF_OFFSET)
                | Item.getIdFromItem(input.getItem())
            )) {
            return true;
        }

        return BLACK_LIST.contains(
            (input.getItemDamage() << Platform.DEF_OFFSET)
            | Item.getIdFromItem(input.getItem())
        );
    }

    private boolean isEmpty(final IMEInventory<IAEItemStack> meInventory) {
        return meInventory.getAvailableItems(AEApi.instance().storage().createItemList())
            .isEmpty();
    }
    ...
    public IAEItemStack injectItems(
        ...
        if (this.canHoldNewItem()) // room for new type, and for at least one item!
        {
            final int remainingItemCount
                = (int) this.getRemainingItemCount() - this.getBytesPerType() * 8;
            ...
            if (remainingItemCount > 0) {
                if (input.getStackSize() > remainingItemCount) {
                    ...
                    if (mode == Actionable.MODULATE) {
                        final ItemStack toWrite
                            = Platform.cloneItemStack(sharedItemStack);
    ...
    private void saveChanges() {
        ...
        for (final IAEItemStack v : this.cellItems) {
            ...
            /*
             * NBTBase tagSlotCount = tagCompound.getTag( itemSlotCount[x] ); if (
             * tagSlotCount instanceof NBTTagInt ) ((NBTTagInt) tagSlotCount).data = (int)
             * v.getStackSize(); else
             */
        ...
        /*
         * if ( tagType instanceof NBTTagShort ) ((NBTTagShort) tagType).data =
         * storedItems = (short) cellItems.size(); else
         */
        ...
        /*
         * if ( tagCount instanceof NBTTagInt ) ((NBTTagInt) tagCount).data =
         * storedItemCount = itemCount; else
         */
    ...
    private void loadCellItems() {
        ...
        for (int x = 0; x < types; x++) {
            final ItemStack t = ItemStack.loadItemStackFromNBT(
    ...
    public long getUsedBytes() {
        final long bytesForItemCount
            = (this.getStoredItemCount() + this.getUnusedItemCount()) / 8;
