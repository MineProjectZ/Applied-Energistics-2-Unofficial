...
public class SecurityInventory implements IMEInventoryHandler<IAEItemStack> {
    ...
    public IAEItemStack injectItems(
        ...
        if (this.hasPermission(src)) {
            if (AEApi.instance().definitions().items().biometricCard().isSameAs(
                    input.getItemStack()
                )) {
    ...
    private boolean hasPermission(final BaseActionSource src) {
        if (src.isPlayer()) {
            try {
                return this.securityTile.getProxy().getSecurity().hasPermission(
                    ((PlayerSource) src).player, SecurityPermissions.SECURITY
                );
