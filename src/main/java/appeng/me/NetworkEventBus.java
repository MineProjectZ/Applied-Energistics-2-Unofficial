...
public class NetworkEventBus {
    ...
    private static final
        Map<Class<? extends MENetworkEvent>, Map<Class, MENetworkEventInfo>> EVENTS
    ...
    void readClass(final Class listAs, final Class c) {
        ...
        try {
            for (final Method m : c.getMethods()) {
                final MENetworkEventSubscribe s
                    = m.getAnnotation(MENetworkEventSubscribe.class);
                if (s != null) {
                    final Class[] types = m.getParameterTypes();
                    if (types.length == 1) {
                        if (MENetworkEvent.class.isAssignableFrom(types[0])) {
                            Map<Class, MENetworkEventInfo> classEvents
                                = EVENTS.get(types[0]);
                            if (classEvents == null) {
                                EVENTS.put(
                                    types[0],
            ...
            throw new IllegalStateException(
                "Error while adding " + c.getName() + " to event bus", t
            );
        }
    }
    ...
    MENetworkEvent postEvent(final Grid g, final MENetworkEvent e) {
        ...
        try {
            if (subscribers != null) {
                for (final Entry<Class, MENetworkEventInfo> subscriber :
    ...
    MENetworkEvent
    postEventTo(final Grid grid, final GridNode node, final MENetworkEvent e) {
        final Map<Class, MENetworkEventInfo> subscribers = EVENTS.get(e.getClass());
        int x = 0;
    ...
    private class EventMethod {
        ...
        public EventMethod(
            final Class Event, final Class ObjClass, final Method ObjMethod
        ) {
        ...
        private void invoke(final Object obj, final MENetworkEvent e)
            throws NetworkEventDone {
            try {
    ...
    private class MENetworkEventInfo {
        ...
        private void
        Add(final Class Event, final Class ObjClass, final Method ObjMethod) {
