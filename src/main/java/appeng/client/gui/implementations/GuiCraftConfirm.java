...
public class GuiCraftConfirm extends AEBaseGui {
    ...
    public GuiCraftConfirm(
        final InventoryPlayer inventoryPlayer, final ITerminalHost te
    ) {
    ...
    private void updateCPUButtonText() {
        String btnTextText
            = GuiText.CraftingCPU.getLocal() + ": " + GuiText.Automatic.getLocal();
        if (this.ccc.getSelectedCpu()
            >= 0) // && status.selectedCpu < status.cpus.size() )
        {
            if (this.ccc.getName().length() > 0) {
                final String name = this.ccc.getName().substring(
                    0, Math.min(20, this.ccc.getName().length())
                );
                btnTextText = GuiText.CraftingCPU.getLocal() + ": " + name;
            } else {
                btnTextText
                    = GuiText.CraftingCPU.getLocal() + ": #" + this.ccc.getSelectedCpu();
            }
        }
    ...
    drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        ...
        this.fontRendererObj.drawString(
            GuiText.CraftingPlan.getLocal() + " - " + Add, 8, 7, 4210752
        );
            ...
            if (refStack != null) {
                ...
                if (stored != null && stored.getStackSize() > 0) {
                    ...
                    this.fontRendererObj.drawString(
                        str,
                        (int
                        ) ((x * (1 + sectionLength) + xo + sectionLength - 19 - (w * 0.5))
                           * 2),
                ...
                if (missingStack != null && missingStack.getStackSize() > 0) {
                    ...
                    if (this.tooltip == z - viewStart) {
                        lineList.add(
                            ...
                            + Long.toString(missingStack.getStackSize())
                        );
                    }
                ...
                if (red) {
                    ...
                    drawRect(
                        startX, startY, startX + sectionLength, startY + offY, 0x1AFF0000
                    );
                }
    ...
    protected void actionPerformed(final GuiButton btn) {
        ...
        if (btn == this.start) {
            try {
                NetworkHandler.instance.sendToServer(
                    new PacketValueConfig("Terminal.Start", "Start")
                );
