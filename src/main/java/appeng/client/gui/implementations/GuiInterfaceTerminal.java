...
public class GuiInterfaceTerminal extends AEBaseGui {
    ...
    private final HashMap<Long, ClientDCInternalInv> byId
        = new HashMap<Long, ClientDCInternalInv>();
    ...
    public GuiInterfaceTerminal(
        final InventoryPlayer inventoryPlayer, final PartInterfaceTerminal te
    ) {
    ...
    public void initGui() {
        ...
        this.searchField = new MEGuiTextField(
            this.fontRendererObj,
            this.guiLeft + Math.max(104, this.offsetX),
            this.guiTop + 4,
            65,
            12
        );
    ...
    drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        this.fontRendererObj.drawString(
            this.getGuiDisplayName(GuiText.InterfaceTerminal.getLocal()), 8, 6, 4210752
        );
        this.fontRendererObj.drawString(
            ...
            if (lineObj instanceof ClientDCInternalInv) {
                ...
                for (int z = 0; z < inv.getInventory().getSizeInventory(); z++) {
                    this.inventorySlots.inventorySlots.add(
                        new SlotDisconnected(inv, z, z * 18 + 8, 1 + offset)
                    );
                }
                ...
                while (name.length() > 2
                       && this.fontRendererObj.getStringWidth(name) > 155) {
        ...
        for (int x = 0; x < LINES_ON_PAGE && ex + x < this.lines.size(); x++) {
            ...
            if (lineObj instanceof ClientDCInternalInv) {
                ...
                this.drawTexturedModalRect(
                    offsetX + 7, offsetY + offset, 7, 139, width, 18
                );
            }
            offset += 18;
        }
        ...
        for (final Object oKey : in.func_150296_c()) {
            ...
            if (key.startsWith("=")) {
                try {
                    ...
                    final ClientDCInternalInv current = this.getById(
                        id, invData.getLong("sortBy"), invData.getString("un")
                    );
    ...
    private void refreshList() {
        ...
        final Set<Object> cachedSearch
            = this.getCacheForSearchTerm(searchFilterLowerCase);
        ...
        for (final ClientDCInternalInv entry : this.byId.values()) {
            ...
            if (!found && !searchFilterLowerCase.isEmpty()) {
                for (final ItemStack itemStack : entry.getInventory()) {
                    found = this.itemStackMatchesSearchTerm(
                        itemStack, searchFilterLowerCase
                    );
                    if (found) {
                        break;
                    }
        ...
        for (final String n : this.names) {
            ...
            final ArrayList<ClientDCInternalInv> clientInventories
                = new ArrayList<ClientDCInternalInv>();
    ...
    private boolean
    itemStackMatchesSearchTerm(final ItemStack itemStack, final String searchTerm) {
        if (itemStack == null) {
            return false;
        }
        ...
        for (int i = 0; i < outTag.tagCount(); i++) {
            final ItemStack parsedItemStack
                = ItemStack.loadItemStackFromNBT(outTag.getCompoundTagAt(i));
    ...
    private Set<Object> getCacheForSearchTerm(final String searchTerm) {
        ...
        if (cache.isEmpty() && searchTerm.length() > 1) {
            cache.addAll(this.getCacheForSearchTerm(
    ...
    private ClientDCInternalInv
    getById(final long id, final long sortBy, final String string) {
