...
public class GuiSecurity extends GuiMEMonitorable {
        ...
        if (toggleSetting != null) {
            try {
                NetworkHandler.instance.sendToServer(new PacketValueConfig(
                    "TileSecurity.ToggleOption", toggleSetting.name()
                ));
    ...
    protected String getBackground() {
        ...
        this.inject.setState(
            (cs.getPermissionMode() & (1 << SecurityPermissions.INJECT.ordinal())) > 0
        );
        this.extract.setState(
            (cs.getPermissionMode() & (1 << SecurityPermissions.EXTRACT.ordinal())) > 0
        );
        this.craft.setState(
            (cs.getPermissionMode() & (1 << SecurityPermissions.CRAFT.ordinal())) > 0
        );
        this.build.setState(
            (cs.getPermissionMode() & (1 << SecurityPermissions.BUILD.ordinal())) > 0
        );
        this.security.setState(
            (cs.getPermissionMode() & (1 << SecurityPermissions.SECURITY.ordinal())) > 0
        );
