...
public class GuiFormationPlane extends GuiUpgradeable {
    ...
    public GuiFormationPlane(
        final InventoryPlayer inventoryPlayer, final PartFormationPlane te
    ) {
    ...
    protected void addButtons() {
        this.placeMode = new GuiImgButton(
            this.guiLeft - 18, this.guiTop + 28, Settings.PLACE_BLOCK, YesNo.YES
        );
    ...
    drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        this.fontRendererObj.drawString(
            this.getGuiDisplayName(GuiText.FormationPlane.getLocal()), 8, 6, 4210752
        );
        this.fontRendererObj.drawString(
    ...
    protected void actionPerformed(final GuiButton btn) {
        ...
        if (btn == this.priority) {
            ...
            NetworkHandler.instance.sendToServer(
                new PacketConfigButton(this.placeMode.getSetting(), backwards)
            );
        }
    }
}
