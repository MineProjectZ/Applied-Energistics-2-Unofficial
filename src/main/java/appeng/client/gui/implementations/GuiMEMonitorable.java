...
public class GuiMEMonitorable
    extends AEBaseMEGui implements ISortSource, IConfigManagerHost {
    ...
    public void initGui() {
        ...
        for (int y = 0; y < this.rows; y++) {
            for (int x = 0; x < this.perRow; x++) {
                this.getMeSlots().add(new InternalSlotME(
                    this.repo, x + y * this.perRow, this.offsetX + x * 18, 18 + y * 18
                ));
            }
        }

        if (AEConfig.instance.getConfigManager().getSetting(Settings.TERMINAL_STYLE)
            != TerminalStyle.FULL) {
    ...
    protected void mouseClicked(final int xCoord, final int yCoord, final int btn) {
        ...
        if (searchMode != SearchBoxMode.AUTOSEARCH
            && searchMode != SearchBoxMode.NEI_AUTOSEARCH) {
    ...
    drawBG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        ...
        for (int x = 0; x < this.rows; x++) {
            this.drawTexturedModalRect(
                offsetX, offsetY + 18 + x * 18, 0, 18, x_width, 18
            );
        }
    ...
    int getMaxRows() {
        return AEConfig.instance.getConfigManager().getSetting(Settings.TERMINAL_STYLE)
                == TerminalStyle.SMALL
            ? 6
            : Integer.MAX_VALUE;
    }
