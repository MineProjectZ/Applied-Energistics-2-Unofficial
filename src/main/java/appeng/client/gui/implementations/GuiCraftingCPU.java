...
public class GuiCraftingCPU extends AEBaseGui implements ISortSource {
        ...
        if (this.cancel == btn) {
            try {
                NetworkHandler.instance.sendToServer(
                    new PacketValueConfig("TileCrafting.Cancel", "Cancel")
                );
    ...
    private void setScrollBar() {
        ...
        this.getScrollBar()
            .setTop(SCROLLBAR_TOP)
            .setLeft(SCROLLBAR_LEFT)
            .setHeight(SCROLLBAR_HEIGHT);
    ...
    drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        ...
        if (this.craftingCpu.getEstimatedTime() > 0 && !this.visual.isEmpty()) {
            final long etaInMilliseconds = TimeUnit.MILLISECONDS.convert(
                this.craftingCpu.getEstimatedTime(), TimeUnit.NANOSECONDS
            );
            final String etaTimeText = DurationFormatUtils.formatDuration(
                etaInMilliseconds, GuiText.ETAFormat.getLocal()
            );
            title += " - " + etaTimeText;
        }

        this.fontRendererObj.drawString(
            title, TITLE_LEFT_OFFSET, TITLE_TOP_OFFSET, TEXT_COLOR
        );
        ...
        for (int z = viewStart; z < Math.min(viewEnd, this.visual.size()); z++) {
            final IAEItemStack refStack
                = this.visual.get(z); // repo.getReferenceItem( z );
            if (refStack != null) {
                GL11.glPushMatrix();
                ...
                if (AEConfig.instance.useColoredCraftingStatus && (active || scheduled)) {
                    final int bgColor = (active ? AEColor.Green.blackVariant
                                                : AEColor.Yellow.blackVariant)
                        | BACKGROUND_ALPHA;
                    final int startX
                        = (x * (1 + SECTION_LENGTH) + ITEMSTACK_LEFT_OFFSET) * 2;
                    final int startY = ((y * offY + ITEMSTACK_TOP_OFFSET) - 3) * 2;
                    drawRect(
                        startX,
                        startY,
                        startX + (SECTION_LENGTH * 2),
                        startY + (offY * 2) - 2,
                        bgColor
                    );
                }
                ...
                if (stored != null && stored.getStackSize() > 0) {
                    ...
                    this.fontRendererObj.drawString(
                        str,
                        (int
                        ) ((x * (1 + SECTION_LENGTH) + ITEMSTACK_LEFT_OFFSET
                            + SECTION_LENGTH - 19 - (w * 0.5))
                           * 2),
                    ...
                    if (this.tooltip == z - viewStart) {
                        lineList.add(
                            ...
                            + Long.toString(stored.getStackSize())
                        );
                    }
                ...
                if (activeStack != null && activeStack.getStackSize() > 0) {
                    ...
                    if (this.tooltip == z - viewStart) {
                        lineList.add(
                            ...
                            + Long.toString(activeStack.getStackSize())
                        );
                    }
                ...
                if (pendingStack != null && pendingStack.getStackSize() > 0) {
                    ...
                    if (this.tooltip == z - viewStart) {
                        lineList.add(
                            ...
                            + Long.toString(pendingStack.getStackSize())
                        );
                    }
                }

                GL11.glPopMatrix();
                final int posX = x * (1 + SECTION_LENGTH) + ITEMSTACK_LEFT_OFFSET
                    + SECTION_LENGTH - 19;
                ...
                if (this.tooltip == z - viewStart) {
                    ...
                    toolPosX = x * (1 + SECTION_LENGTH) + ITEMSTACK_LEFT_OFFSET
                        + SECTION_LENGTH - 8;
