...
public abstract class AEBaseGui extends GuiContainer {
    ...
    join(final Collection<String> toolTip, final String delimiter) {
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float btn) {
        ...
        for (final Object c : this.buttonList) {
            if (c instanceof ITooltip) {
                ...
                if (x < mouseX && x + tooltip.getWidth() > mouseX
                    && tooltip.isVisible()) {
    ...
    public void drawTooltip(
        final int par2, final int par3, final int forceWidth, final String message
    ) {
            ...
            if (this.drag_click.size() > 1) {
                for (final Slot dr : this.drag_click) {
                    final PacketInventoryAction p = new PacketInventoryAction(
                        c == 0 ? InventoryAction.PICKUP_OR_SET_DOWN
    ...
    protected void handleMouseClick(
        final Slot slot, final int slotIdx, final int ctrlDown, final int mouseButton
    ) {
        final EntityPlayer player = Minecraft.getMinecraft().thePlayer;

        if (slot instanceof SlotFake) {
            final InventoryAction action = ctrlDown == 1
        ...
        if (slot instanceof SlotPatternTerm) {
            ...
            try {
                NetworkHandler.instance.sendToServer(
                    ((SlotPatternTerm) slot).getRequest(isShiftKeyDown())
                );
            ...
            if (isShiftKeyDown()) {
                ...
                action = (mouseButton == 1) ? InventoryAction.CRAFT_STACK
                                            : InventoryAction.CRAFT_ITEM;
            }
        ...
        if (slot instanceof SlotDisconnected) {
            ...
            switch (mouseButton) {
                case 0: // pickup / set-down.
                    action = ctrlDown == 1 ? InventoryAction.SPLIT_OR_PLACE_SINGLE
                                           : InventoryAction.PICKUP_OR_SET_DOWN;
                    break;
                case 1:
                    action = ctrlDown == 1 ? InventoryAction.PICKUP_SINGLE
        ...
        if (!this.disableShiftClick && isShiftKeyDown()) {
            ...
            if (this.dbl_whichItem == null || this.bl_clicked != slot
                ...
                if (slot != null) {
                    this.dbl_whichItem
                        = slot.getHasStack() ? slot.getStack().copy() : null;
                } else {
                ...
                for (final Slot inventorySlot : slots) {
                    if (inventorySlot != null
                        ...
                        this.handleMouseClick(
                            inventorySlot, inventorySlot.slotNumber, ctrlDown, 1
                        );
                    }
                }
            }
            ...
            for (int j = 0; j < 9; ++j) {
                if (keyCode == this.mc.gameSettings.keyBindsHotbar[j].getKeyCode()) {
                    ...
                    for (final Slot s : slots) {
                        if (s.getSlotIndex() == j
                            && s.inventory
                                == ((AEBaseContainer) this.inventorySlots)
                                       .getPlayerInv()) {
                            if (!s.canTakeStack(((AEBaseContainer) this.inventorySlots)
                                                    .getPlayerInv()
                                                    .player)) {
                                return false;
                            }
    ...
    public void onGuiClosed() {
        super.onGuiClosed();
        this.subGui
            = true; // in case the gui is reopened later ( i'm looking at you NEI )
    }
        ...
        if (slot instanceof SlotME) {
            ...
            if (item != null) {
                ...
                final InventoryAction direction
                    = wheel > 0 ? InventoryAction.ROLL_DOWN : InventoryAction.ROLL_UP;
                final int times = Math.abs(wheel);
                final int inventorySize = this.getInventorySlots().size();
                for (int h = 0; h < times; h++) {
                    final PacketInventoryAction p
                        = new PacketInventoryAction(direction, inventorySize, 0);
                    NetworkHandler.instance.sendToServer(p);
                }
            }
        }
    ...
    protected void drawItem(final int x, final int y, final ItemStack is) {
        ...
        itemRender.renderItemAndEffectIntoGUI(
            this.fontRendererObj, this.mc.renderEngine, is, x, y
        );
        GL11.glPopAttrib();
    ...
    private void drawSlot(final Slot s) {
        if (s instanceof SlotME) {
            ...
            try {
                ...
                AELog.warn(
                    "[AppEng] AE prevented crash while drawing slot: " + err.toString()
                );
            }
            ...
            try {
                    ...
                    if (aes.getIcon() >= 0) {
                        ...
                        try {
                            ...
                            tessellator.setColorRGBA_F(
                                1.0f, 1.0f, 1.0f, aes.getOpacityOfIcon()
                            );
                ...
                if (is != null && s instanceof AppEngSlot) {
                    if (((AppEngSlot) s).getIsValid()
                        == hasCalculatedValidness.NotAvailable) {
                        boolean isValid = s.isItemValid(is) || s instanceof SlotOutput
                            || s instanceof AppEngCraftingSlot
                            || s instanceof SlotDisabled || s instanceof SlotInaccessible
                            || s instanceof SlotFake || s instanceof SlotRestrictedInput
                            || s instanceof SlotDisconnected;
                        if (isValid && s instanceof SlotRestrictedInput) {
                            try {
                                isValid = ((SlotRestrictedInput) s)
                                              .isValid(is, this.mc.theWorld);
                            } catch (final Exception err) {
                                AELog.debug(err);
                            }
                        }
                        ((AppEngSlot) s)
                            .setIsValid(
                                isValid ? hasCalculatedValidness.Valid
                                        : hasCalculatedValidness.Invalid
                            );
                    }
    ...
    private void safeDrawSlot(final Slot s) {
        try {
            GuiContainer.class.getDeclaredMethod("func_146977_a_original", Slot.class)
                .invoke(this, s);
        } catch (final Exception err) {}
    }
    ...
    public void bindTexture(final String file) {
        final ResourceLocation loc
            = new ResourceLocation(AppEng.MOD_ID, "textures/" + file);
        this.mc.getTextureManager().bindTexture(loc);
    }
