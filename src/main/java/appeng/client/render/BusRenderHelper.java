...
public final class BusRenderHelper implements IPartRenderHelper {
    ...
    public BusRenderHelper() {
        ...
        this.maybeBlock
            = AEApi.instance().definitions().blocks().multiPart().maybeBlock();
    ...
    public void setOrientation(
        final ForgeDirection dx, final ForgeDirection dy, final ForgeDirection dz
    ) {
    ...
    public double[] getBounds() {
        return new double[] { this.minX, this.minY, this.minZ,
    ...
    public ISimplifiedBundle useSimplifiedRendering(
        ...
        if (sim != null && this.maybeBlock.isPresent()
            && rbw.similarLighting(
                this.maybeBlock.get(), rbw.blockAccess, x, y, z, sim
            )) {
            rbw.populate(sim);
    ...
    @Override
    public void renderInventoryFace(
        final IIcon icon, final ForgeDirection face, final RenderBlocks renderer
    ) {
        renderer.setRenderBounds(
            this.minX / 16.0,
            this.minY / 16.0,
    ...
    @Override
    public void
    renderBlock(final int x, final int y, final int z, final RenderBlocks renderer) {
        if (!this.renderThis()) {
            return;
        }
        ...
        for (final Block multiPart :
            ...
            renderer.uvRotateBottom
                = info.getTexture(ForgeDirection.DOWN)
                      .setFlip(
                          BaseBlockRender.getOrientation(ForgeDirection.DOWN, forward, up)
                      );
            renderer.uvRotateTop
                = info.getTexture(ForgeDirection.UP)
                      .setFlip(
                          BaseBlockRender.getOrientation(ForgeDirection.UP, forward, up)
                      );

            renderer.uvRotateEast
                = info.getTexture(ForgeDirection.EAST)
                      .setFlip(
                          BaseBlockRender.getOrientation(ForgeDirection.EAST, forward, up)
                      );
            renderer.uvRotateWest
                = info.getTexture(ForgeDirection.WEST)
                      .setFlip(
                          BaseBlockRender.getOrientation(ForgeDirection.WEST, forward, up)
                      );

            renderer.uvRotateNorth = info.getTexture(ForgeDirection.NORTH)
                                         .setFlip(BaseBlockRender.getOrientation(
                                             ForgeDirection.NORTH, forward, up
                                         ));
            renderer.uvRotateSouth = info.getTexture(ForgeDirection.SOUTH)
                                         .setFlip(BaseBlockRender.getOrientation(
                                             ForgeDirection.SOUTH, forward, up
                                         ));
    ...
    public Block getBlock() {
        ...
        throw new MissingDefinition(
            "Tried to access the multi part block without it being defined."
        );
    }
    ...
    @Override
    public void renderBlockCurrentBounds(
        final int x, final int y, final int z, final RenderBlocks renderer
    ) {
        if (!this.renderThis()) {
            return;
        }
