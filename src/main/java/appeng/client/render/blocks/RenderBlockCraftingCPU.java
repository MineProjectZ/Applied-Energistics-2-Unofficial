...
public class RenderBlockCraftingCPU<B extends BlockCraftingUnit, T
    ...
    public boolean renderInWorld(
        ...
        final IIcon theIcon
            = blk.getIcon(ForgeDirection.SOUTH.ordinal(), meta | (formed ? 8 : 0));
        ...
        if (formed && renderer.overrideBlockTexture == null) {
            ...
            try {
                craftingTile.setLightCache(
                    i.useSimplifiedRendering(x, y, z, null, craftingTile.getLightCache())
                );
            } catch (final Throwable ignored) {}

            final float highX
                = this.isConnected(w, x, y, z, ForgeDirection.EAST) ? 16 : 13.01f;
            final float lowX
                = this.isConnected(w, x, y, z, ForgeDirection.WEST) ? 0 : 2.99f;

            final float highY
                = this.isConnected(w, x, y, z, ForgeDirection.UP) ? 16 : 13.01f;
            final float lowY
                = this.isConnected(w, x, y, z, ForgeDirection.DOWN) ? 0 : 2.99f;

            final float highZ
                = this.isConnected(w, x, y, z, ForgeDirection.SOUTH) ? 16 : 13.01f;
            final float lowZ
                = this.isConnected(w, x, y, z, ForgeDirection.NORTH) ? 0 : 2.99f;
    ...
    private boolean isConnected(
        ...
        if (0 <= tileYPos && tileYPos <= 255) {
            final TileEntity tile
                = w.getTileEntity(x + side.offsetX, tileYPos, z + side.offsetZ);
    ...
    private float
    fso(final ForgeDirection side, final float def, final ForgeDirection target) {
        if (side == target) {
    ...
    private void handleSide(
        ...
        for (final ForgeDirection a : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if ((side.offsetX != 0 || side.offsetZ != 0)
                && (a == ForgeDirection.NORTH || a == ForgeDirection.EAST
                    || a == ForgeDirection.WEST || a == ForgeDirection.SOUTH)) {
    ...
    private float
    gso(final ForgeDirection side, final float def, final ForgeDirection target) {
        if (side != target) {
            if (side.offsetX > 0 || side.offsetY > 0 || side.offsetZ > 0) {
                return 16;
            }
