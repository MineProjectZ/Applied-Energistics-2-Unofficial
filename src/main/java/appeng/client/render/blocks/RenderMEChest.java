...
public class RenderMEChest extends BaseBlockRender<BlockChest, TileChest> {
    ...
    public boolean renderInWorld(
        ...
        int b = world.getLightBrightnessForSkyBlocks(
            x + forward.offsetX, y + forward.offsetY, z + forward.offsetZ, 0
        );
        Tessellator.instance.setBrightness(b);
        Tessellator.instance.setColorOpaque_I(0xffffff);

        final int offsetU = -4;
        final FlippableIcon flippableIcon = new FlippableIcon(new OffsetIcon(
            ExtraBlockTextures.MEStorageCellTextures.getIcon(), offsetU, offsetV
        ));

        if (forward == ForgeDirection.EAST
            && (up == ForgeDirection.NORTH || up == ForgeDirection.SOUTH)) {
        ...
        if (stat != 0) {
            ...
            this.renderFace(
                x, y, z, imb, ExtraBlockTextures.White.getIcon(), renderer, forward
            );
        }

        b = world.getLightBrightnessForSkyBlocks(
        ...
        final ICellHandler ch
            = AEApi.instance().registries().cell().getHandler(sp.getStorageType());
        ...
        this.renderFace(
            ...
            z,
            imb,
            ico == null ? ExtraBlockTextures.MEChest.getIcon() : ico,
            renderer,
            up
        );
