...
public class RenderBlockSkyCompass
    extends BaseBlockRender<BlockSkyCompass, TileSkyCompass> {
    ...
    public void renderInventory(
        ...
        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON
            ...
            final CompassResult cr
                = CompassManager.INSTANCE.getCompassDirection(0, x, y, z);

            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    CompassManager.INSTANCE.getCompassDirection(
                        0, x + i - 1, y, z + j - 1
                    );
                }
            }
            ...
            if (cr.isValidResult()) {
                if (cr.isSpin()) {
                    ...
                    if (type == ItemRenderType.EQUIPPED_FIRST_PERSON) {
                        ...
                        ) this.flipidiy(cr.getRad() + offRads + adjustment));
                    } else {
                        final float offRads = rYaw / 180.0f * (float) Math.PI;
                        final float adjustment = (float) Math.PI * -0.74f;
    ...
    public void renderTile(
        ...
        if (skyCompass.getForward() == ForgeDirection.UP
