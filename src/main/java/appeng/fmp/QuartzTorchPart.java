...
public class QuartzTorchPart extends McSidedMetaPart implements IRandomDisplayTick {
    ...
    public static McBlockPart
    placement(final World world, BlockCoord pos, final int side) {
        pos = pos.copy().offset(side);
        if (!world.isSideSolid(
                pos.x, pos.y, pos.z, ForgeDirection.getOrientation(side)
            )) {
            return null;
        }
    ...
    public Block getBlock() {
        for (final Block torchBlock :
             AEApi.instance().definitions().blocks().quartzTorch().maybeBlock().asSet()) {
            return torchBlock;
        }

        throw new MissingDefinition(
            "Tried to retrieve a quartz torch, even though it is disabled."
        );
    }
}