...
public class FMPEvent {
    ...
    private static boolean place(final EntityPlayer player, final World world) {
        ...
        final BlockCoord pos
            = new BlockCoord(hit.blockX, hit.blockY, hit.blockZ).offset(hit.sideHit);
        ...
        if (world.isRemote
            && !player.isSneaking(
            )) // attempt to use block activated like normal and tell the server
        // the right stuff
        {
            final Vector3 f
                = new Vector3(hit.hitVec).add(-hit.blockX, -hit.blockY, -hit.blockZ);
