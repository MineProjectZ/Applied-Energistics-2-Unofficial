...
public class ChunkLogger implements ISubCommand {
    ...
    public void onChunkLoadEvent(final ChunkEvent.Load event) {
        if (!event.world.isRemote) {
            AELog.info(
                "Chunk Loaded:   " + event.getChunk().xPosition + ", "
    ...
    public void onChunkUnloadEvent(final ChunkEvent.Unload unload) {
        if (!unload.world.isRemote) {
            AELog.info(
                "Chunk Unloaded: " + unload.getChunk().xPosition + ", "
                + unload.getChunk().zPosition
            );
            this.displayStack();
        }
    }
    ...
    @Override
    public void
    call(final MinecraftServer srv, final String[] data, final ICommandSender sender) {
        this.enabled = !this.enabled;

        if (this.enabled) {
            MinecraftForge.EVENT_BUS.register(this);
            sender.addChatMessage(
                new ChatComponentTranslation("commands.ae2.ChunkLoggerOn")
            );
        } else {
            MinecraftForge.EVENT_BUS.unregister(this);
            sender.addChatMessage(
                new ChatComponentTranslation("commands.ae2.ChunkLoggerOff")
            );
        }
    }
}
