        ...
        if (player.isSneaking()) {
            ...
            if (ish != null) {
                ...
                if (te instanceof IGridHost) {
                    ...
                    final ForgeDirection sideOff
                        = ForgeDirection.getOrientation(src_side);
                    final ForgeDirection currentSideOff
                    ...
                    if (n != null) {
                        ...
                        if (g != null) {
                            ...
                            if (sc.isValidRegion()) {
                                ...
                                for (int i = 1; i < scale_x; i++) {
                                    for (int j = 1; j < scale_y; j++) {
                                        for (int k = 1; k < scale_z; k++) {
                                            final Block blk = src_w.getBlock(
                                                min_x + i, min_y + j, min_z + k
                                            );
                                            final int meta = src_w.getBlockMetadata(
                                                min_x + i, min_y + j, min_z + k
                                            );
                                            world.setBlock(
                                                i + rel_x,
                                            ...
                                            if (blk != null && blk.hasTileEntity(meta)) {
                                                ...
                                                final TileEntity nte
                                                    = blk.createTileEntity(world, meta);
                                                final NBTTagCompound data
                                                    = new NBTTagCompound();
                                                ote.writeToNBT(data);
                                                nte.readFromNBT((NBTTagCompound
                                                ) data.copy());
                                                world.setTileEntity(
                                                    i + rel_x, j + rel_y, k + rel_z, nte
                                                );
                                            }
                                            world.markBlockForUpdate(
                                                i + rel_x, j + rel_y, k + rel_z
                                            );
                                        }
                                    }
                                }
                                ...
                                this.outputMsg(
                                    player, "requires valid spatial pylon setup."
                                );
                            }
                        } else {
